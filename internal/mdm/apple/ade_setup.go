...
func (s *Store) ADEDeviceEnrollment(ctx context.Context, scope Scope, id string) (*ADEDeviceEnrollment, error) {
	...
	err := s.db.QueryRowContext(ctx, `SELECT a.server_id,a.profile_id,p.name,a.setup_state,a.setup_error,p.removable,a.created_at,a.expires_at,a.awaiting_configuration,a.setup_updated_at,(a.setup_state IN ('awaiting','releasing') AND COALESCE(a.awaiting_configuration,false) AND NOT EXISTS(SELECT 1 FROM mdm_apple_commands c WHERE c.tenant_id=a.tenant_id AND c.device_id=a.device_id AND c.ade_setup AND c.attempts>0)) FROM mdm_apple_ade_admissions a JOIN mdm_apple_ade_profiles p ON p.id=a.profile_id AND p.tenant_id=a.tenant_id JOIN mdm_apple_devices d ON d.id=a.device_id AND d.tenant_id=a.tenant_id WHERE a.tenant_id=$1 AND a.device_id=$2 AND ($3=0 OR d.site_id=$3)`, scope.TenantID, id, scope.SiteID).Scan(&v.ServerID, &v.ProfileID, &v.ProfileName, &v.SetupState, &v.SetupError, &v.Removable, &v.CreatedAt, &v.ExpiresAt, &v.AwaitingConfiguration, &v.SetupUpdatedAt, &v.CanChangeApplications)
...
func (s *Store) reconcileADESetup(ctx context.Context, tx *sql.Tx, d *Device) error {
	...
	ssoReady, err := s.reconcileADEPlatformSSO(ctx, tx, d)
	if err != nil {
		return err
	}
	var ready bool
