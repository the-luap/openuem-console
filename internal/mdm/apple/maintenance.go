...
func (s *Store) expireCommands(ctx context.Context) error {
	...
	if _, err = tx.ExecContext(ctx, `UPDATE mdm_apple_commands SET payload='\x' WHERE (filevault OR recovery_lock) AND status NOT IN ('queued','sent','not_now') AND octet_length(payload)>0`); err != nil {
		return err
	}
	return tx.Commit()
}
