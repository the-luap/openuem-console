...
func (s *Store) assignWithADERequirement(ctx context.Context, tx *sql.Tx, d *Device, p *Profile, desired, requirement string) error {
	...
	if desired == "installed" {
		if err := validateVPNProfile(p, current); err != nil {
			return fmt.Errorf("%w: %v", ErrProfilePrerequisite, err)
		}
		if err := validateADCertificateProfile(p, current); err != nil {
			return fmt.Errorf("%w: %v", ErrProfilePrerequisite, err)
		}
		if err := validateWiFiProfile(p, current); err != nil {
			return fmt.Errorf("%w: %v", ErrProfilePrerequisite, err)
		}
		...
		if err := validatePublicCertificateProfile(p, current); err != nil {
			return fmt.Errorf("%w: %v", ErrProfilePrerequisite, err)
		}
		if err := validatePrivacyProfile(p, current); err != nil {
			return fmt.Errorf("%w: %v", ErrProfilePrerequisite, err)
		}
