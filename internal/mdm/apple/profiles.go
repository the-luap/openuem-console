...
func ParseProfile(data []byte) (*Profile, error) {
	...
	for _, item := range items {
		...
		if err := validateADCertificatePayload(p, scope, nil); err != nil {
			return nil, err
		}
		...
		if err := validateSCEPCertificatePayload(p, scope, nil); err != nil {
			return nil, err
		}
		if err := validatePublicCertificatePayload(p, scope, nil); err != nil {
			return nil, err
		}
...
func BuildProfile(name, identifier, kind string, settings map[string]any) ([]byte, error) {
	...
	case "apple-scep":
		if err := buildSCEPCertificatePayload(payload, settings, scope); err != nil {
			return nil, err
		}
	case "apple-certificates":
		var err error
		additional, err = buildPublicCertificatePayload(payload, settings, scope)
		if err != nil {
			return nil, err
		}
	case "macos-privacy":
