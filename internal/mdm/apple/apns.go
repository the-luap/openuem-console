...
func (s *Store) Run(ctx context.Context, logger *slog.Logger) {
	...
	for {
		...
		for _, task := range []func(context.Context) error{s.CleanupEnrollmentClaims, s.CleanupPushRequests, s.expireCommands, s.ScheduleInventory, s.ReconcileUpdateAvailability} {
