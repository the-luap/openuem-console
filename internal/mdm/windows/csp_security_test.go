...
func TestCSPMigrationPreservesExistingSessions(t *testing.T) {
	...
	if _, err := f.store.db.Exec(`DROP TABLE mdm_windows_csp_observations,mdm_windows_csp_audit,mdm_windows_csp_commands; DROP FUNCTION mdm_windows_keep_csp_command(); ALTER TABLE mdm_windows_syncml_packets DROP CONSTRAINT mdm_windows_syncml_packet_scope_message; DELETE FROM mdm_windows_migrations WHERE name='migrations/005_csp_commands.sql'`); err != nil {
		t.Fatal(err)
	}
	for n := 0; n < 2; n++ {
