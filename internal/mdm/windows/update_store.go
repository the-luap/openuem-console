...
func (s *Store) openUpdateRun(run *updateStoredRun) (*updateIntent, error) {
	...
	if decodeSyncMLProtectedJSON(plain, &intent) != nil || intent.Version != 1 || len(intent.Name) > 128 || !validEnrollmentUsername(intent.Name) || intent.Policy.Validate() != nil {
		return nil, ErrAuthoritySecret
	}
	return &intent, nil
}

func updateRunCommands(intent *updateIntent, remove bool) ([3]CSPCommandSpec, error) {
	configure, verify, err := UpdatePolicyCommands(intent.Policy, remove)
	return [3]CSPCommandSpec{updatePlatformCommand(), configure, verify}, err
}
...
// three generated trees can use ManageUpdates; callers cannot supply raw CSP.
func (s *Store) EnqueueUpdatePolicy(ctx context.Context, actor string, scope access.Scope, deviceID, requestKey, name string, policy UpdatePolicy, remove bool, validFor time.Duration) (*UpdateRun, error) {
	...
	intent := &updateIntent{Version: 1, Name: name, Policy: policy}
...
func (s *Store) updateRunForCommand(ctx context.Context, tx *sql.Tx, c *cspStoredCommand) (*updateStoredRun, *updateIntent, error) {
	if !canonicalInvitationID(c.UpdateRunID) || c.UpdateStep < 0 || c.UpdateStep > 2 {
