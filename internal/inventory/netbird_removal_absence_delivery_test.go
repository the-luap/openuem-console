...
func absenceDeliveryFixture(t *testing.T) (*removalResolutionFixture, *inventory.NetbirdRemovalAbsenceStore, *inventory.NetbirdRemovalAbsence) {
	t.Helper()
	x := absenceFixture(t)
	s := absenceStore(t, x, nil)
	r := absenceRequest(t, x, s, absenceReview(t, x, s))
	return x, s, r
}
func absenceDeliveryStore(t *testing.T, x *removalResolutionFixture, control inventory.NetbirdOperationControl, execute inventory.NetbirdRemovalAbsenceExecutor) *inventory.NetbirdRemovalAbsenceStore {
	t.Helper()
	if control == nil {
		control = absenceControl(t, x)
	}
	s, err := inventory.NewNetbirdRemovalAbsenceDeliveryStore(x.f.db, x.f.permissions, true, control, execute)
	require.NoError(t, err)
	return s
}

func TestNetbirdAbsenceDeliveryCommitsExactCommandAndVerifiedCompletion(t *testing.T) {
	...
	s := absenceDeliveryStore(t, x, nil, func(ctx context.Context, c netbirdcommand.Command) (*netbirdcommand.Receipt, error) {
		...
		require.NotEqual(t, c.Revision, c.RemovalAbsence.JournalRevision)
		require.Equal(t, r.JournalRevision, c.RemovalAbsence.JournalRevision)
		require.Equal(t, 2*time.Minute, c.ExpiresAt.Sub(c.IssuedAt))
		require.True(t, c.ExpiresAt.After(r.ExpiresAt))
		var hash string
		require.NoError(t, f.db.QueryRowContext(ctx, `SELECT command_hash FROM uem_netbird_removal_absence_attempts WHERE request_id=$1`, r.ID).Scan(&hash))
		actual, err := c.Digest()
		require.NoError(t, err)
		require.Equal(t, actual, hash)
		var count int
		require.NoError(t, f.db.QueryRowContext(ctx, `SELECT count(*) FROM uem_netbird_operations_audit WHERE request_id=$1 AND resource_id LIKE '%/verify-removal-absence/deliver'`, r.ID).Scan(&count))
		...
		_, err = f.db.ExecContext(ctx, `UPDATE uem_netbird_removal_absences SET cancellation_id=$2,cancelled_by='tag-admin',cancelled_at=clock_timestamp() WHERE id=$1`, r.ID, uuid.NewString())
		require.Error(t, err)
		for _, query := range []string{
			`UPDATE uem_netbird_removal_absences SET completed_at=clock_timestamp() WHERE id=$1`,
			`UPDATE uem_netbird_removal_absence_attempts SET command_hash=repeat('a',64) WHERE request_id=$1`,
			`DELETE FROM uem_netbird_removal_absence_attempts WHERE request_id=$1`,
		} {
		...
		require.NoError(t, err)
		_, err = f.db.ExecContext(ctx, `INSERT INTO uem_netbird_removal_absence_results(request_id,outcome,receipt) VALUES($1,'completed',$2::jsonb)`, r.ID, string(wrongData))
		require.Error(t, err, "uncertain receipt manufactured completion")
		receipt, err := netbirdcommand.ReceiptFor(c, "completed")
		return &receipt, err
	})
	d, err := s.Verify(t.Context(), "tag-admin", f.scope, f.id, r.ID, r.Revision)
	require.NoError(t, err)
	...
	got, err := s.Verify(t.Context(), "tag-admin", f.scope, f.id, r.ID, r.Revision)
	require.NoError(t, err)
	...
	_, err = requests.Request(t.Context(), "tag-admin", f.scope, f.id, x.r.ID, uuid.NewString(), r.AbsenceDigest, r.Revision)
	require.NoError(t, err, "completed removal retained device exclusion")
}

func TestNetbirdAbsenceDeliveryRechecksReviewBeforeCreatingAttempt(t *testing.T) {
	for _, kind := range []string{"state", "absent", "journal", "certificate", "consumer", "revoked", "permission", "cancelled", "actor", "revision", "audit"} {
		t.Run(kind, func(t *testing.T) {
			...
			control := func(ctx context.Context, c netbirdcommand.ControlRequest) (*netbirdcommand.ControlResponse, error) {
				response, err := absenceControl(t, x)(ctx, c)
				if err != nil {
					return nil, err
				}
				switch kind {
				case "state":
					response.RemovalAbsence.StateDigest = strings.Repeat("a", 64)
				case "absent":
					response.Outcome = "absent"
					response.RemovalAbsence = netbirdcommand.RemovalAbsence{}
				case "journal":
					response.State.Revision = strings.Repeat("a", 64)
					response.RemovalAbsence.JournalRevision = response.State.Revision
				}
				return response, nil
			}
			calls := 0
			...
			case "audit":
				_, err = f.db.ExecContext(ctx, `ALTER TABLE uem_netbird_operations_audit ADD CONSTRAINT owned_removal_attempt_failure CHECK(resource_id NOT LIKE '%/verify-removal-absence/deliver') NOT VALID`)
			}
			require.NoError(t, err)
			_, err = s.Verify(ctx, actor, f.scope, f.id, r.ID, revision)
			require.Error(t, err)
			require.Zero(t, calls)
			var count int
			require.NoError(t, f.db.QueryRowContext(ctx, `SELECT count(*) FROM uem_netbird_removal_absence_attempts`).Scan(&count))
			require.Zero(t, count)
		})
	}
}

func TestNetbirdAbsenceDeliveryRetainsAllUncertainResultsWithoutRetry(t *testing.T) {
		...
		t.Run(kind, func(t *testing.T) {
			...
			s := absenceDeliveryStore(t, x, nil, func(ctx context.Context, c netbirdcommand.Command) (*netbirdcommand.Receipt, error) {
				...
				if kind == "result-audit" {
					_, e := f.db.ExecContext(ctx, `ALTER TABLE uem_netbird_operations_audit ADD CONSTRAINT owned_removal_result_failure CHECK(resource_id NOT LIKE '%/verify-removal-absence/result-%') NOT VALID`)
					require.NoError(t, e)
				}
				return &response, err
			})
			d, err := s.Verify(ctx, "tag-admin", f.scope, f.id, r.ID, r.Revision)
			if kind == "result-audit" {
				require.Error(t, err)
				require.Nil(t, d)
			...
			require.ErrorIs(t, err, inventory.ErrNetbirdOperationConflict)
			replayed, err := s.Verify(t.Context(), "tag-admin", f.scope, f.id, r.ID, r.Revision)
			require.NoError(t, err)
			if kind == "result-audit" {
...
func TestNetbirdAbsenceConcurrentDeliveryReturnsPendingAndExcludesCancellation(t *testing.T) {
	x, requests, r := absenceDeliveryFixture(t)
	f := x.f
	...
	go func() { _, err := s.Verify(ctx, "tag-admin", f.scope, f.id, r.ID, r.Revision); done <- err }()
	select {
	case <-entered:
	case <-ctx.Done():
		t.Fatal(ctx.Err())
	}
	pending, err := s.Verify(ctx, "tag-admin", f.scope, f.id, r.ID, r.Revision)
	require.NoError(t, err)
...
func TestNetbirdAbsenceObservationRecoversOriginalReceiptUnderCurrentCertificate(t *testing.T) {
		...
		t.Run(kind, func(t *testing.T) {
			...
			control := func(ctx context.Context, c netbirdcommand.ControlRequest) (*netbirdcommand.ControlResponse, error) {
				if c.Kind == "removal-absence-state" {
					return absenceControl(t, x)(ctx, c)
				}
				observations.Add(1)
			...
			s := absenceDeliveryStore(t, x, control, func(ctx context.Context, c netbirdcommand.Command) (*netbirdcommand.Receipt, error) {
				calls.Add(1)
				original = c
				if kind == "result-pending" {
					_, err := f.db.ExecContext(ctx, `ALTER TABLE uem_netbird_operations_audit ADD CONSTRAINT owned_removal_observation_failure CHECK(resource_id NOT LIKE '%/verify-removal-absence/result-%') NOT VALID`)
					require.NoError(t, err)
				}
				return nil, errors.New("owned lost reply")
			})
			initial, err := s.Verify(t.Context(), "tag-admin", f.scope, f.id, r.ID, r.Revision)
			if kind == "result-pending" {
				require.Error(t, err)
			...
			require.NoError(t, err)
			observed, err := s.ObserveAbsence(t.Context(), "operator", f.scope, f.id, r.ID, r.Revision)
			require.NoError(t, err)
			...
			var count int
			require.NoError(t, f.db.QueryRowContext(t.Context(), `SELECT count(*) FROM uem_netbird_removal_absence_observations WHERE request_id=$1`, r.ID).Scan(&count))
			require.Equal(t, 1, count)
			replayed, err := s.Verify(t.Context(), "tag-admin", f.scope, f.id, r.ID, r.Revision)
			require.NoError(t, err)
...
func TestNetbirdAbsenceObservationCompletionSurvivesLateUnconfirmedDelivery(t *testing.T) {
	x, _, r := absenceDeliveryFixture(t)
	f := x.f
	...
	s := absenceDeliveryStore(t, x, func(ctx context.Context, c netbirdcommand.ControlRequest) (*netbirdcommand.ControlResponse, error) {
		if c.Kind == "removal-absence-state" {
			return absenceControl(t, x)(ctx, c)
		}
		queries.Add(1)
	...
	again, err := s.ObserveAbsence(ctx, "operator", f.scope, f.id, r.ID, r.Revision)
	require.NoError(t, err)
...
func TestNetbirdAbsenceObservationAuditAndScopeCannotManufactureCompletion(t *testing.T) {
	x, _, r := absenceDeliveryFixture(t)
	f := x.f
	var command netbirdcommand.Command
	var queries atomic.Int32
	...
	for _, kind := range []string{"scope", "permission", "revision", "device", "request"} {
		...
		_, err := s.ObserveAbsence(t.Context(), actor, scope, device, id, revision)
		require.Error(t, err, kind)
	}
	require.Zero(t, queries.Load())
	_, err = f.db.ExecContext(t.Context(), `ALTER TABLE uem_netbird_operations_audit ADD CONSTRAINT owned_absence_observe_failure CHECK(action<>'inventory.netbird.resolution.observe') NOT VALID`)
	require.NoError(t, err)
	_, err = s.ObserveAbsence(t.Context(), "operator", f.scope, f.id, r.ID, r.Revision)
	require.Error(t, err)
	got, err := s.ReadDelivery(t.Context(), "viewer", f.scope, f.id, r.ID)
	require.NoError(t, err)
	require.Equal(t, d, got)
	var count int
	require.NoError(t, f.db.QueryRowContext(t.Context(), `SELECT count(*) FROM uem_netbird_removal_absence_observations`).Scan(&count))
	require.Zero(t, count)
	_, err = f.db.ExecContext(t.Context(), `ALTER TABLE uem_netbird_operations_audit DROP CONSTRAINT owned_absence_observe_failure`)
	require.NoError(t, err)
	got, err = s.ObserveAbsence(t.Context(), "operator", f.scope, f.id, r.ID, r.Revision)
	require.NoError(t, err)
	require.Equal(t, "completed", got.Outcome)
	require.Equal(t, "unconfirmed", got.OriginalOutcome)
}

func TestNetbirdAbsenceDeliveryStartupAndRetainedEvidenceGuards(t *testing.T) {
	x, _, r := absenceDeliveryFixture(t)
	f := x.f
	s := absenceDeliveryStore(t, x, nil, func(_ context.Context, c netbirdcommand.Command) (*netbirdcommand.Receipt, error) {
	...
	_, err = f.db.ExecContext(t.Context(), `ALTER TABLE uem_netbird_removal_absences DISABLE TRIGGER uem_netbird_removal_absence_completion_valid`)
	require.NoError(t, err)
	require.Error(t, inventory.Migrate(t.Context(), f.db))
	_, err = f.db.ExecContext(t.Context(), `ALTER TABLE uem_netbird_removal_absences ENABLE TRIGGER uem_netbird_removal_absence_completion_valid`)
	require.NoError(t, err)
	...
	_, err = f.db.ExecContext(t.Context(), `ALTER TABLE uem_netbird_removal_absence_attempts DISABLE TRIGGER uem_netbird_removal_absence_attempt_immutable`)
	require.NoError(t, err)
	_, err = f.db.ExecContext(t.Context(), `UPDATE uem_netbird_removal_absence_attempts SET certificate_hash=repeat('e',64)`)
	require.NoError(t, err)
	_, err = f.db.ExecContext(t.Context(), `ALTER TABLE uem_netbird_removal_absence_attempts ENABLE TRIGGER uem_netbird_removal_absence_attempt_immutable`)
	require.NoError(t, err)
	var calls atomic.Int32
...
func TestNetbirdAbsenceObservationSQLRejectsForeignAndMalformedEvidence(t *testing.T) {
	x, _, r := absenceDeliveryFixture(t)
	f := x.f
	var command netbirdcommand.Command
	s := absenceDeliveryStore(t, x, nil, func(_ context.Context, c netbirdcommand.Command) (*netbirdcommand.Receipt, error) {
	...
	for _, kind := range []string{"control-operation", "control-reference", "control-revision", "control-source", "receipt-operation", "receipt-reference", "receipt-revision", "receipt-hash", "receipt-null", "receipt-source", "response-hash", "response-source", "release", "outcome", "timestamp", "valid"} {
		q := netbirdcommand.ControlRequest{Version: netbirdcommand.RecoveryVersion, Identity: command.Identity, RequestID: uuid.NewString(), Kind: "receipt", ReferenceID: r.ID, CommandHash: d.CommandHash, Revision: r.Revision, Operation: "verify-removal-absence", IssuedAt: time.Now().UTC(), ExpiresAt: time.Now().UTC().Add(2 * time.Second)}
		...
		require.NoError(t, err)
		_, err = f.db.ExecContext(t.Context(), `INSERT INTO uem_netbird_removal_absence_observations(id,request_id,actor,control_hash,control,response) VALUES($1,$2,'operator',$3,$4::jsonb,$5::jsonb)`, q.RequestID, r.ID, hash, string(control), string(response))
		if kind == "valid" {
			require.NoError(t, err)
		} else {
			require.Error(t, err, kind)
	...
	for _, query := range []string{`UPDATE uem_netbird_removal_absence_observations SET actor='admin'`, `DELETE FROM uem_netbird_removal_absence_observations`} {
		_, err := f.db.ExecContext(t.Context(), query)
		require.Error(t, err)
	}
}

func TestNetbirdAbsenceDeliveryExpiryDoesNotGrantReplayOrFreeDevice(t *testing.T) {
	for _, attempted := range []bool{false, true} {
		t.Run(map[bool]string{false: "unattempted", true: "retained"}[attempted], func(t *testing.T) {
			...
			require.NoError(t, err)
			_, err = f.db.ExecContext(t.Context(), `UPDATE uem_netbird_removal_absences SET expires_at=clock_timestamp()`)
			require.NoError(t, err)
