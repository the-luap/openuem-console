...
type recoveryResolutionFixture struct {
	original                   *removalResolutionFixture
	f                          *refreshFixture
	s, requests                *inventory.NetbirdRemovalRecoveryStore
	nativeAbsent               bool
	inspections                int
	r                          *inventory.NetbirdRemovalRecovery
...
func newRecoveryResolutionFixture(t *testing.T, status string) *recoveryResolutionFixture {
	t.Helper()
	original, requests, request := recoveryDeliveryFixture(t)
	f := original.f
	x := &recoveryResolutionFixture{original: original, f: f, requests: requests, status: status, canRelease: true, journalRevision: strings.Repeat("a", 64)}
	x.r = request
	x.s = recoveryDeliveryStore(t, original, func(ctx context.Context, c netbirdcommand.ControlRequest) (*netbirdcommand.ControlResponse, error) {
		if c.Kind == "removal-recovery-state" {
			x.inspections++
			reply, err := recoveryControl(t, original)(ctx, c)
			if err == nil && x.nativeAbsent {
				reply.Outcome = "absent"
				reply.RemovalRecovery = netbirdcommand.RemovalRecovery{}
			}
			return reply, err
		}
		if c.Kind == "state" {
		...
		if outcome == "ok" {
			hash, e := x.command.Digest()
			require.NoError(t, e)
			p.Receipt = netbirdcommand.Receipt{Version: 1, RequestID: x.r.ID, DeviceID: f.id, Revision: x.r.Revision, CommandHash: hash, Operation: "recover-removal", Status: x.status}
	...
	_, err := x.s.Recover(t.Context(), "tag-admin", f.scope, f.id, x.r.ID, x.r.Revision)
	require.NoError(t, err)
	return x
}

func (x *recoveryResolutionFixture) review(t *testing.T) *inventory.NetbirdRemovalRecoveryResolutionReview {
	t.Helper()
	v, err := x.s.ReviewRecoveryResolution(t.Context(), "operator", x.f.scope, x.f.id, x.r.ID, x.r.Revision)
	require.NoError(t, err)
	return v
}
func (x *recoveryResolutionFixture) resolve(t *testing.T, v *inventory.NetbirdRemovalRecoveryResolutionReview) *inventory.NetbirdRemovalRecoveryResolution {
	t.Helper()
	d, err := x.s.ResolveRecovery(t.Context(), "operator", x.f.scope, x.f.id, x.r.ID, x.r.Revision, v.ResolutionID, v.Revision)
	require.NoError(t, err)
	return d
}
func (x *recoveryResolutionFixture) reconcile(t *testing.T, id string) *inventory.NetbirdRemovalRecoveryResolution {
	t.Helper()
	d, err := x.s.ReconcileRecoveryResolution(t.Context(), "operator", x.f.scope, x.f.id, x.r.ID, x.r.Revision, id)
	require.NoError(t, err)
	return d
}

func TestNetbirdRecoveryResolutionCommitsReviewedControlBeforeTransport(t *testing.T) {
	for _, status := range []string{"missing", "unconfirmed"} {
		t.Run(status, func(t *testing.T) {
			x := newRecoveryResolutionFixture(t, status)
			v := x.review(t)
			kind := "withdraw"
			version := 2
			...
			x.beforeMutation = func(ctx context.Context, c netbirdcommand.ControlRequest) {
				...
				require.NoError(t, x.f.db.QueryRowContext(ctx, `SELECT control::text,control_hash FROM uem_netbird_removal_recovery_controls WHERE request_id=$1`, x.r.ID).Scan(&retained, &hash))
				...
				read, err := x.s.ReadRecoveryResolution(ctx, "viewer", x.f.scope, x.f.id, x.r.ID, x.r.Revision)
				require.NoError(t, err)
			...
			// A later independent recovery still references the original uninstall.
			nextStore := recoveryStore(t, x.original, func(ctx context.Context, c netbirdcommand.ControlRequest) (*netbirdcommand.ControlResponse, error) {
				p, err := recoveryControl(t, x.original)(ctx, c)
				if err == nil {
			...
			current := recoveryReview(t, x.original, nextStore)
			require.NotEqual(t, x.r.JournalRevision, current.Journal.Revision)
			fresh, err := nextStore.Request(t.Context(), "tag-admin", x.f.scope, x.f.id, x.original.r.ID, uuid.NewString(), current.RecoveryDigest, current.Revision)
			require.NoError(t, err)
...
func TestNetbirdRecoveryResolutionRecoversLostReplyUnderCurrentAuthority(t *testing.T) {
	for _, status := range []string{"missing", "unconfirmed"} {
		t.Run(status, func(t *testing.T) {
			x := newRecoveryResolutionFixture(t, status)
			v := x.review(t)
			...
			observed, err := x.s.ObserveRecovery(t.Context(), "operator", x.f.scope, x.f.id, x.r.ID, x.r.Revision)
			require.NoError(t, err)
			require.Equal(t, 1, x.inspections, "receipt recovery repeated native-state inspection")
			...
			var outcome string
			require.NoError(t, x.f.db.QueryRowContext(t.Context(), `SELECT outcome FROM uem_netbird_removal_recovery_results WHERE request_id=$1`, x.r.ID).Scan(&outcome))
			require.Equal(t, "unconfirmed", outcome)
		})
	}
}

func TestNetbirdRecoveryResolutionRetriesOnlyNewReviewedControl(t *testing.T) {
...
func TestNetbirdRecoveryResolutionRejectsChangedOrExpiredReview(t *testing.T) {
	for _, change := range []string{"certificate", "consumer", "permission", "scope", "revision", "journal", "receipt", "expiry", "audit"} {
		t.Run(change, func(t *testing.T) {
			x := newRecoveryResolutionFixture(t, "missing")
			v := x.review(t)
			scope := x.f.scope
			...
			case "receipt":
				x.status = "unconfirmed"
			case "expiry":
				_, err = x.f.db.ExecContext(t.Context(), `ALTER TABLE uem_netbird_removal_recovery_reviews DISABLE TRIGGER uem_netbird_removal_recovery_review_immutable`)
				require.NoError(t, err)
				_, err = x.f.db.ExecContext(t.Context(), `UPDATE uem_netbird_removal_recovery_reviews SET created_at=created_at-interval '3 minutes',expires_at=expires_at-interval '3 minutes'`)
			case "audit":
			...
			require.NoError(t, err)
			_, err = x.s.ResolveRecovery(t.Context(), "operator", scope, x.f.id, x.r.ID, revision, v.ResolutionID, v.Revision)
			require.Error(t, err)
			require.Zero(t, x.mutations)
			var n int
			require.NoError(t, x.f.db.QueryRowContext(t.Context(), `SELECT count(*) FROM uem_netbird_removal_recovery_controls`).Scan(&n))
			require.Zero(t, n)
		})
	}
}

func TestNetbirdRecoveryResolutionKeepsBarrierForUnownedOrIneligibleEvidence(t *testing.T) {
	for _, condition := range []string{"orphan", "foreign", "missing-after-release", "released-without-release-attempt", "wrong-operation", "wrong-hash", "wrong-query", "unavailable", "audit"} {
		t.Run(condition, func(t *testing.T) {
			...
			got, err := x.s.ReconcileRecoveryResolution(t.Context(), "operator", x.f.scope, x.f.id, x.r.ID, x.r.Revision, d.ID)
			if condition == "audit" {
				require.Error(t, err)
			} else {
				require.NoError(t, err)
			...
			_, err = x.f.db.ExecContext(t.Context(), `UPDATE uem_netbird_removal_recoveries SET released_at=clock_timestamp(),released_by='operator',resolution_id=$2 WHERE id=$1`, x.r.ID, d.ID)
			require.Error(t, err)
			require.Equal(t, 1, x.mutations)
		})
	}
...
func TestNetbirdRecoveryResolutionRecoversCompletionAndAtomicAuditFailure(t *testing.T) {
	for _, completion := range []bool{false, true} {
		t.Run(map[bool]string{false: "release", true: "completion"}[completion], func(t *testing.T) {
			...
			if completion {
				...
				require.Nil(t, d)
				var n int
				require.NoError(t, x.f.db.QueryRowContext(t.Context(), `SELECT count(*) FROM uem_netbird_removal_recovery_control_results`).Scan(&n))
				require.Zero(t, n)
...
func TestNetbirdRecoveryResolutionIndependentSQLProofAndStartupGuards(t *testing.T) {
	x := newRecoveryResolutionFixture(t, "unconfirmed")
	v := x.review(t)
	...
	require.NoError(t, err)
	_, err = x.s.ResolveRecovery(t.Context(), "operator", x.f.scope, x.f.id, x.r.ID, x.r.Revision, v.ResolutionID, v.Revision)
	require.Error(t, err)
	_, err = x.f.db.ExecContext(t.Context(), `ALTER TABLE uem_netbird_operations_audit DROP CONSTRAINT owned_removal_proof_audit`)
	require.NoError(t, err)
	var wire []byte
	var attempt string
	require.NoError(t, x.f.db.QueryRowContext(t.Context(), `SELECT id::text,control FROM uem_netbird_removal_recovery_controls WHERE request_id=$1`, x.r.ID).Scan(&attempt, &wire))
	c, err := netbirdcommand.DecodeControl(wire)
	require.NoError(t, err)
	p, err := netbirdcommand.ControlResponseFor(c, "ok")
	require.NoError(t, err)
	p.Receipt = netbirdcommand.Receipt{Version: 1, RequestID: x.r.ID, DeviceID: x.f.id, Revision: x.r.Revision, CommandHash: c.CommandHash, Operation: "recover-removal", Status: "unconfirmed"}
	...
	for _, change := range []string{"status", "release", "revision", "certificate", "hash", "kind", "extra", "state", "missing-release"} {
		...
		require.NoError(t, e)
		_, err = x.f.db.ExecContext(t.Context(), `INSERT INTO uem_netbird_removal_recovery_control_results(attempt_id,response) VALUES($1,$2::jsonb)`, attempt, string(altered))
		require.Error(t, err, change)
	}
	var n int
	require.NoError(t, x.f.db.QueryRowContext(t.Context(), `SELECT count(*) FROM uem_netbird_removal_recovery_control_results`).Scan(&n))
	require.Zero(t, n)
	_, err = x.f.db.ExecContext(t.Context(), `INSERT INTO uem_netbird_removal_recovery_control_results(attempt_id,response) VALUES($1,$2::jsonb)`, attempt, string(encoded))
	require.NoError(t, err)
	for _, resolution := range []string{uuid.NewString(), x.r.ID} {
		_, err = x.f.db.ExecContext(t.Context(), `INSERT INTO uem_netbird_removal_recovery_release_proofs(request_id,resolution_id,actor,attempt_id) VALUES($1,$2,'operator',$3)`, x.r.ID, resolution, attempt)
		require.Error(t, err)
	}
	...
	require.NoError(t, x.f.db.QueryRowContext(t.Context(), `INSERT INTO uem_netbird_removal_recovery_release_proofs(request_id,resolution_id,actor,attempt_id) VALUES($1,$2,'operator',$3) RETURNING recorded_at`, x.r.ID, v.ResolutionID, attempt).Scan(&recorded))
	_, err = x.f.db.ExecContext(t.Context(), `UPDATE uem_netbird_removal_recoveries SET released_at=$2,released_by='viewer',resolution_id=$3 WHERE id=$1`, x.r.ID, recorded, v.ResolutionID)
	require.Error(t, err)
	_, err = x.f.db.ExecContext(t.Context(), `UPDATE uem_netbird_removal_recoveries SET released_at=$2,released_by='operator',resolution_id=$3 WHERE id=$1`, x.r.ID, recorded, v.ResolutionID)
	require.NoError(t, err)
	...
	for _, q := range []string{`UPDATE uem_netbird_removal_recoveries SET released_at=NULL,released_by=NULL,resolution_id=NULL`, `UPDATE uem_netbird_removal_recoveries SET completed_at=clock_timestamp()`} {
		_, err = x.f.db.ExecContext(t.Context(), q)
		require.Error(t, err)
	}
	_, err = x.f.db.ExecContext(t.Context(), `ALTER TABLE uem_netbird_removal_recoveries DISABLE TRIGGER uem_netbird_removal_recovery_release_required`)
	require.NoError(t, err)
	require.Error(t, inventory.Migrate(t.Context(), x.f.db))
	_, err = x.f.db.ExecContext(t.Context(), `ALTER TABLE uem_netbird_removal_recoveries ENABLE TRIGGER uem_netbird_removal_recovery_release_required`)
	require.NoError(t, err)
	require.NoError(t, inventory.Migrate(t.Context(), x.f.db))
}

func TestNetbirdRecoveryResolutionIndependentSQLControlAdmission(t *testing.T) {
	x := newRecoveryResolutionFixture(t, "missing")
	v := x.review(t)
	var reviewID string
	require.NoError(t, x.f.db.QueryRowContext(t.Context(), `SELECT id::text FROM uem_netbird_removal_recovery_reviews WHERE revision=$1`, v.Revision).Scan(&reviewID))
	_, err := x.f.db.ExecContext(t.Context(), `INSERT INTO uem_netbird_removal_recovery_resolutions(request_id,id,actor,review_id) VALUES($1,$2,'operator',$3)`, x.r.ID, v.ResolutionID, reviewID)
	require.NoError(t, err)
	var issued time.Time
	require.NoError(t, x.f.db.QueryRowContext(t.Context(), `SELECT clock_timestamp()`).Scan(&issued))
	hash, err := x.command.Digest()
	require.NoError(t, err)
	c := netbirdcommand.ControlRequest{Version: 2, Identity: x.command.Identity, RequestID: v.ResolutionID, Kind: "withdraw", ReferenceID: x.r.ID, CommandHash: hash, Revision: x.r.Revision, Operation: "recover-removal", IssuedAt: issued, ExpiresAt: issued.Add(5 * time.Second)}
	...
	require.NoError(t, e)
	response.Receipt = netbirdcommand.Receipt{Version: 1, RequestID: x.r.ID, DeviceID: x.f.id, Revision: x.r.Revision, CommandHash: hash, Operation: "recover-removal", Status: "withdrawn"}
	response.ReleaseID = v.ResolutionID
	proof, e := netbirdcommand.EncodeControlResponse(query, response)
	require.NoError(t, e)
	_, err = x.f.db.ExecContext(t.Context(), `INSERT INTO uem_netbird_removal_recovery_observations(id,request_id,actor,control_hash,control,response) VALUES($1,$2,'operator',$3,$4::jsonb,$5::jsonb)`, query.RequestID, x.r.ID, qhash, string(qwire), string(proof))
	require.NoError(t, err)
	insert := `INSERT INTO uem_netbird_removal_recovery_controls(id,request_id,resolution_id,review_id,actor,sequence,kind,control_hash,control) VALUES($1,$2,$3,$4,'operator',1,'withdraw',$5,$6::jsonb)`
	...
	require.NoError(t, err)
	_, err = x.f.db.ExecContext(t.Context(), `INSERT INTO uem_netbird_removal_recovery_release_proofs(request_id,resolution_id,actor,observation_id) VALUES($1,$2,'operator',$3)`, x.r.ID, v.ResolutionID, query.RequestID)
	require.Error(t, err)
	require.Zero(t, x.mutations)
}

func TestNetbirdRecoveryResolutionRejectsOriginalUninstallEvidenceAndUUIDs(t *testing.T) {
		...
		t.Run(field, func(t *testing.T) {
			...
			x.beforeMutation = func(context.Context, netbirdcommand.ControlRequest) {
				x.responseChange = func(p *netbirdcommand.ControlResponse) {
					...
					case "hash":
						p.Receipt.CommandHash = x.r.Recovery.Original.CommandHash
					}
				}
			}
			d := x.resolve(t, v)
...
func TestNetbirdRecoveryResolutionConcurrentConsumedReviewReadsPending(t *testing.T) {
	x := newRecoveryResolutionFixture(t, "unconfirmed")
	v := x.review(t)
	...
	type result struct {
		d   *inventory.NetbirdRemovalRecoveryResolution
		err error
	}
	done := make(chan result, 1)
	go func() {
		d, err := x.s.ResolveRecovery(ctx, "operator", x.f.scope, x.f.id, x.r.ID, x.r.Revision, v.ResolutionID, v.Revision)
		done <- result{d, err}
	}()
	select {
	case <-entered:
	...
	pending, err := x.s.ResolveRecovery(ctx, "operator", x.f.scope, x.f.id, x.r.ID, x.r.Revision, v.ResolutionID, v.Revision)
	require.NoError(t, err)
...
func TestNetbirdRecoveryResolutionReleaseSurvivesLateNativeResult(t *testing.T) {
	for _, status := range []string{"unconfirmed", "completed"} {
		t.Run(status, func(t *testing.T) {
			original, requests, r := recoveryDeliveryFixture(t)
			f := original.f
			...
			defer cancel()
			s := recoveryDeliveryStore(t, original, func(ctx context.Context, c netbirdcommand.ControlRequest) (*netbirdcommand.ControlResponse, error) {
				if c.Kind == "removal-recovery-state" {
					return recoveryControl(t, original)(ctx, c)
				}
				p, err := netbirdcommand.ControlResponseFor(c, "ok")
				if c.Kind == "state" {
			...
			v, err := s.ReviewRecoveryResolution(ctx, "operator", f.scope, f.id, r.ID, r.Revision)
			require.NoError(t, err)
			require.Equal(t, "release", v.Kind)
			d, err := s.ResolveRecovery(ctx, "operator", f.scope, f.id, r.ID, r.Revision, v.ResolutionID, v.Revision)
			require.NoError(t, err)
			require.NotNil(t, d.ConfirmedAt)
			close(release)
			var got result
