...
func TestNetbirdStageCleanupDispatchDeliversOnceAcrossRestart(t *testing.T) {
	...
	execute := func(ctx context.Context, c netbirdcommand.Command) (*netbirdcommand.Receipt, error) {
		...
		require.Nil(t, stop)
		var admitted int
		require.NoError(t, f.db.QueryRowContext(ctx, `SELECT count(*) FROM uem_netbird_removal_stage_cleanup_attempts WHERE request_id=$1`, r.ID).Scan(&admitted))
		require.Equal(t, 1, admitted)
		_, err = f.db.ExecContext(ctx, `INSERT INTO uem_netbird_removal_stage_cleanup_dispatch_stops(request_id,reason) VALUES($1,'source_changed')`, r.ID)
		require.ErrorContains(t, err, "cannot stop before delivery")
		out, err := netbirdcommand.ReceiptFor(c, "completed")
		return &out, err
	}
	s := stageCleanupDeliveryStore(t, x, nil, execute)
	...
	restarted := stageCleanupDeliveryStore(t, x, nil, execute)
...
func TestNetbirdStageCleanupDispatchRetainsUncertainAttemptsWithoutRetry(t *testing.T) {
	for _, outcome := range []string{"lost", "rejected", "result-audit"} {
		t.Run(outcome, func(t *testing.T) {
			...
			if outcome == "result-audit" {
				_, err := f.db.ExecContext(t.Context(), `ALTER TABLE uem_netbird_operations_audit ADD CONSTRAINT owned_removal_dispatch_audit CHECK(resource_id NOT LIKE '%/cleanup-removal-stage/result-unconfirmed') NOT VALID`)
				require.NoError(t, err)
			}
			worked, err := s.DispatchOne(t.Context())
			require.True(t, worked)
...
func TestNetbirdStageCleanupDispatchStopsChangedPreflightWithoutClearingBarrier(t *testing.T) {
	for _, change := range []string{"permission", "certificate", "descriptor", "absent", "unavailable", "stop-audit"} {
		t.Run(change, func(t *testing.T) {
			...
			control := func(ctx context.Context, c netbirdcommand.ControlRequest) (*netbirdcommand.ControlResponse, error) {
				...
				p, err := stageCleanupControl(t, x)(ctx, c)
				if err == nil && change == "descriptor" {
					p.RemovalStageCleanup.StateDigest = strings.Repeat("e", 64)
				}
				if change == "absent" {
					p.Outcome = "absent"
					p.RemovalStageCleanup = netbirdcommand.RemovalStageCleanup{}
				}
				return p, err
			}
			...
			_, err = requests.Cancel(t.Context(), "operator", f.scope, f.id, r.ID, r.Revision, uuid.NewString())
			require.NoError(t, err)
			for _, query := range []string{`DELETE FROM uem_netbird_removal_stage_cleanup_dispatch_stops`, `UPDATE uem_netbird_removal_stage_cleanup_dispatch_stops SET reason='expired'`} {
...
func TestNetbirdStageCleanupDispatchJoinsShutdownAndConcurrentDelivery(t *testing.T) {
	x, requests, r := stageCleanupDeliveryFixture(t)
	f := x.f
...
func TestNetbirdStageCleanupDispatchStopGuardsNativeAdmissionAndStartup(t *testing.T) {
	...
	_, err := f.db.ExecContext(t.Context(), `INSERT INTO uem_netbird_removal_stage_cleanup_dispatch_stops(request_id,reason) VALUES($1,'source_changed')`, r.ID)
	require.NoError(t, err)
	_, err = s.Cleanup(t.Context(), "tag-admin", f.scope, f.id, r.ID, r.Revision)
	require.ErrorIs(t, err, inventory.ErrNetbirdOperationConflict)
	require.Zero(t, calls.Load())
	_, err = f.db.ExecContext(t.Context(), `WITH stamp AS(SELECT clock_timestamp() AS at) INSERT INTO uem_netbird_removal_stage_cleanup_attempts(request_id,wire_version,certificate_hash,command_hash,issued_at,expires_at) SELECT $1,7,certificate_hash,repeat('f',64),stamp.at,stamp.at+interval '1 minute' FROM uem_agent_identities,stamp WHERE id=$2`, r.ID, f.id)
	...
	for _, guard := range [][2]string{{"uem_netbird_removal_stage_cleanup_dispatch_stops", "uem_netbird_removal_stage_cleanup_dispatch_stop_immutable"}, {"uem_netbird_removal_stage_cleanup_dispatch_stops", "uem_netbird_removal_stage_cleanup_dispatch_stop_valid"}, {"uem_netbird_removal_stage_cleanup_attempts", "uem_netbird_removal_stage_cleanup_dispatch_required"}} {
...
func TestNetbirdStageCleanupDispatchExpiresWithoutNativeInspection(t *testing.T) {
	x, _, r := stageCleanupDeliveryFixture(t)
	f := x.f
	id := r.ID
	...
	require.NoError(t, err)
	_, err = f.db.ExecContext(t.Context(), `UPDATE uem_netbird_removal_stage_cleanups SET expires_at=clock_timestamp() WHERE id=$1`, r.ID)
	require.NoError(t, err)
