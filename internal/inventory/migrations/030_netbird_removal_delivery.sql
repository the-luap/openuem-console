ALTER TABLE uem_netbird_removals ADD COLUMN completed_at TIMESTAMPTZ;
ALTER TABLE uem_netbird_removals ADD CONSTRAINT uem_netbird_removal_completion_time
...
CREATE TABLE uem_netbird_removal_attempts (
 request_id UUID PRIMARY KEY REFERENCES uem_netbird_removals(id),
...
CREATE TABLE uem_netbird_removal_results (
...
CREATE TABLE uem_netbird_removal_observations (
...
CREATE INDEX uem_netbird_removal_observations_history ON uem_netbird_removal_observations(request_id,recorded_at DESC,id DESC);
CREATE FUNCTION uem_netbird_removal_attempt_immutable() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN RAISE EXCEPTION 'NetBird removal attempts are permanent'; END $$;
CREATE TRIGGER uem_netbird_removal_attempt_immutable BEFORE UPDATE OR DELETE ON uem_netbird_removal_attempts
 FOR EACH ROW EXECUTE FUNCTION uem_netbird_removal_attempt_immutable();
CREATE FUNCTION uem_netbird_removal_result_immutable() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN RAISE EXCEPTION 'NetBird removal results are permanent'; END $$;
CREATE TRIGGER uem_netbird_removal_result_immutable BEFORE UPDATE OR DELETE ON uem_netbird_removal_results
 FOR EACH ROW EXECUTE FUNCTION uem_netbird_removal_result_immutable();
CREATE FUNCTION uem_netbird_removal_observation_immutable() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN RAISE EXCEPTION 'NetBird removal observations are permanent'; END $$;
CREATE TRIGGER uem_netbird_removal_observation_immutable BEFORE UPDATE OR DELETE ON uem_netbird_removal_observations
 FOR EACH ROW EXECUTE FUNCTION uem_netbird_removal_observation_immutable();

CREATE FUNCTION uem_netbird_removal_attempt_valid() RETURNS trigger LANGUAGE plpgsql AS $$
DECLARE r uem_netbird_removals%ROWTYPE; valid BOOLEAN;
BEGIN
 ...
 THEN RAISE EXCEPTION 'NetBird removal request is unavailable'; END IF;
 IF NEW.issued_at<r.requested_at OR NEW.issued_at>clock_timestamp()+interval '5 seconds'
  OR NEW.expires_at<=clock_timestamp()
 THEN RAISE EXCEPTION 'NetBird removal attempt requires fresh command times'; END IF;
 ...
 IF NOT valid THEN RAISE EXCEPTION 'NetBird removal recipient changed'; END IF;
 RETURN NEW;
END $$;
CREATE TRIGGER uem_netbird_removal_attempt_valid BEFORE INSERT ON uem_netbird_removal_attempts
 FOR EACH ROW EXECUTE FUNCTION uem_netbird_removal_attempt_valid();

CREATE FUNCTION uem_netbird_removal_result_valid() RETURNS trigger LANGUAGE plpgsql AS $$
DECLARE r uem_netbird_removals%ROWTYPE; a uem_netbird_removal_attempts%ROWTYPE; status TEXT;
BEGIN
 SELECT * INTO r FROM uem_netbird_removals WHERE id=NEW.request_id FOR UPDATE;
 SELECT * INTO a FROM uem_netbird_removal_attempts WHERE request_id=NEW.request_id;
 IF NOT FOUND OR NEW.recorded_at<a.issued_at OR NEW.recorded_at>clock_timestamp()+interval '5 seconds'
 THEN RAISE EXCEPTION 'NetBird removal result has no matching attempt'; END IF;
 IF NEW.receipt IS NOT NULL THEN
  ...
  IF NOT coalesce(status IN ('completed','unconfirmed','busy','rejected','withdrawn'),false)
   OR NEW.receipt IS DISTINCT FROM jsonb_build_object('version',1,'request_id',r.id::text,'device_id',r.device_id,
    'revision',r.revision,'command_hash',a.command_hash,'operation','uninstall','status',status)
  THEN RAISE EXCEPTION 'NetBird removal receipt does not match'; END IF;
 END IF;
 IF (NEW.outcome='completed') IS DISTINCT FROM coalesce(status='completed',false)
 THEN RAISE EXCEPTION 'NetBird removal outcome has no matching receipt'; END IF;
 RETURN NEW;
END $$;
CREATE TRIGGER uem_netbird_removal_result_valid BEFORE INSERT ON uem_netbird_removal_results
 FOR EACH ROW EXECUTE FUNCTION uem_netbird_removal_result_valid();

CREATE FUNCTION uem_netbird_removal_observation_valid() RETURNS trigger LANGUAGE plpgsql AS $$
DECLARE r uem_netbird_removals%ROWTYPE; a uem_netbird_removal_attempts%ROWTYPE; c JSONB; p JSONB; empty_state JSONB; receipt JSONB; status TEXT;
BEGIN
 SELECT * INTO r FROM uem_netbird_removals WHERE id=NEW.request_id FOR UPDATE;
 SELECT * INTO a FROM uem_netbird_removal_attempts WHERE request_id=NEW.request_id;
 IF NOT FOUND THEN RAISE EXCEPTION 'NetBird removal observation has no attempt'; END IF;
 ...
 IF c IS DISTINCT FROM jsonb_build_object('version',2,'device_id',r.device_id,'tenant_id',r.tenant_id,'site_id',r.site_id,
   ...
   'command_hash',a.command_hash,'issued_at',c->'issued_at','expires_at',c->'expires_at','revision',r.revision,'operation','uninstall')
 ...
 THEN RAISE EXCEPTION 'NetBird removal observation control is invalid'; END IF;
 ...
 THEN RAISE EXCEPTION 'NetBird removal observation time is invalid'; END IF;
 ...
 IF p->>'outcome'='ok' THEN
  ...
  IF NOT coalesce(status IN ('completed','unconfirmed','withdrawn'),false) THEN RAISE EXCEPTION 'NetBird removal observation receipt is invalid'; END IF;
  receipt=jsonb_build_object('version',1,'request_id',r.id::text,'device_id',r.device_id,'revision',r.revision,'command_hash',a.command_hash,'operation','uninstall','status',status);
  ...
  THEN RAISE EXCEPTION 'NetBird removal release evidence is invalid'; END IF;
 ELSE
  IF NOT coalesce(p->>'outcome' IN ('missing','blocked','conflict','unavailable'),false) OR p->>'release_id'<>''
  THEN RAISE EXCEPTION 'NetBird removal observation outcome is invalid'; END IF;
 ...
 THEN RAISE EXCEPTION 'NetBird removal observation response does not match'; END IF;
 RETURN NEW;
END $$;
CREATE TRIGGER uem_netbird_removal_observation_valid BEFORE INSERT ON uem_netbird_removal_observations
 FOR EACH ROW EXECUTE FUNCTION uem_netbird_removal_observation_valid();
...
BEGIN
 ...
 THEN RAISE EXCEPTION 'NetBird removal intent and completion are immutable'; END IF;
 IF NEW.cancelled_at IS NOT NULL AND OLD.cancelled_at IS NULL
  AND EXISTS(SELECT 1 FROM uem_netbird_removal_attempts WHERE request_id=NEW.id)
 THEN RAISE EXCEPTION 'NetBird removal delivery cannot be cancelled'; END IF;
 RETURN NEW;
END $$;
CREATE FUNCTION uem_netbird_removal_completion_valid() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
 IF TG_OP='INSERT' AND NEW.completed_at IS NOT NULL THEN RAISE EXCEPTION 'NetBird removal cannot begin completed'; END IF;
 IF TG_OP='UPDATE' AND OLD.completed_at IS NULL AND NEW.completed_at IS NOT NULL THEN
  IF NOT EXISTS(SELECT 1 FROM uem_netbird_removal_results WHERE request_id=NEW.id AND outcome='completed' AND recorded_at=NEW.completed_at)
   AND NOT EXISTS(SELECT 1 FROM uem_netbird_removal_observations WHERE request_id=NEW.id AND recorded_at=NEW.completed_at
    AND response->>'outcome'='ok' AND response->'receipt'->>'status'='completed' AND response->>'release_id'='')
  THEN RAISE EXCEPTION 'NetBird removal completion requires retained evidence'; END IF;
 END IF;
 RETURN NEW;
END $$;
CREATE TRIGGER uem_netbird_removal_completion_valid BEFORE INSERT OR UPDATE ON uem_netbird_removals
 FOR EACH ROW EXECUTE FUNCTION uem_netbird_removal_completion_valid();
...
BEGIN
 ...
 IF (TG_TABLE_NAME<>'uem_netbird_operations' AND EXISTS(SELECT 1 FROM uem_netbird_operations WHERE id=NEW.id OR (device_id=NEW.device_id AND (status='queued' OR (status='unconfirmed' AND released_at IS NULL)))))
  ...
  OR (TG_TABLE_NAME<>'uem_netbird_removals' AND EXISTS(SELECT 1 FROM uem_netbird_removals WHERE id=NEW.id OR (device_id=NEW.device_id AND cancelled_at IS NULL AND completed_at IS NULL)))
