...
func TestNetbirdStageCleanupStatusAndHistoryRetainScopeWithoutLiveAuthority(t *testing.T) {
	var calls atomic.Int32
	x := stageCleanupFixture(t)
	f := x.f
	requests := stageCleanupStore(t, x, func(ctx context.Context, c netbirdcommand.ControlRequest) (*netbirdcommand.ControlResponse, error) {
		calls.Add(1)
		return stageCleanupControl(t, x)(ctx, c)
	})
	var ids []string
	for range 22 {
		r := stageCleanupRequest(t, x, requests, stageCleanupReview(t, x, requests))
		ids = append(ids, r.ID)
		_, err := requests.Cancel(t.Context(), "operator", f.scope, f.id, r.ID, r.Revision, uuid.NewString())
		require.NoError(t, err)
	}
	last := stageCleanupRequest(t, x, requests, stageCleanupReview(t, x, requests))
	...
	for _, group := range [][]*inventory.NetbirdRemovalStageCleanupStatus{page.Requests, older.Requests} {
		for _, r := range group {
...
func TestNetbirdStageCleanupStatusCombinesReleasedAndOriginalUnconfirmedEvidence(t *testing.T) {
