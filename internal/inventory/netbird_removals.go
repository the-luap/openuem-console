...
func netbirdRequestConflict(ctx context.Context, tx *sql.Tx, device, id string) (bool, error) {
 ...
 OR EXISTS(SELECT 1 FROM uem_netbird_removals WHERE id=$2 OR (device_id=$1 AND cancelled_at IS NULL AND completed_at IS NULL AND released_at IS NULL))`, device, id).Scan(&pending)
	return pending, err
}
