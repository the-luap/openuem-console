...
func TestNetbirdRemovalDispatchDeliversOnceAcrossRestart(t *testing.T) {
	...
	execute := func(ctx context.Context, c netbirdcommand.Command) (*netbirdcommand.Receipt, error) {
		...
		require.Nil(t, stop)
		var admitted int
		require.NoError(t, f.db.QueryRowContext(ctx, `SELECT count(*) FROM uem_netbird_removal_attempts WHERE request_id=$1`, r.ID).Scan(&admitted))
		require.Equal(t, 1, admitted)
		_, err = f.db.ExecContext(ctx, `INSERT INTO uem_netbird_removal_dispatch_stops(request_id,reason) VALUES($1,'source_changed')`, r.ID)
		require.ErrorContains(t, err, "cannot stop before delivery")
		out, err := netbirdcommand.ReceiptFor(c, "completed")
		return &out, err
	}
	s := removalDeliveryStore(t, f, nil, execute)
	...
	restarted := removalDeliveryStore(t, f, nil, execute)
...
func TestNetbirdRemovalDispatchRetainsUncertainAttemptsWithoutRetry(t *testing.T) {
	for _, outcome := range []string{"lost", "rejected", "result-audit"} {
		t.Run(outcome, func(t *testing.T) {
			...
			if outcome == "result-audit" {
				_, err := f.db.ExecContext(t.Context(), `ALTER TABLE uem_netbird_operations_audit ADD CONSTRAINT owned_removal_dispatch_audit CHECK(resource_id NOT LIKE '%/uninstall/result-unconfirmed') NOT VALID`)
				require.NoError(t, err)
			}
			worked, err := s.DispatchOne(t.Context())
			require.True(t, worked)
...
func TestNetbirdRemovalDispatchStopsChangedPreflightWithoutClearingBarrier(t *testing.T) {
	for _, change := range []string{"permission", "certificate", "descriptor", "absent", "unavailable", "stop-audit"} {
		t.Run(change, func(t *testing.T) {
			...
			control := func(ctx context.Context, c netbirdcommand.ControlRequest) (*netbirdcommand.ControlResponse, error) {
				...
				p, err := removalControl(ctx, c)
				if err == nil && change == "descriptor" {
					p.Removal.StateDigest = strings.Repeat("e", 64)
				}
				if change == "absent" {
					p.Outcome = "absent"
					p.Removal = netbirdcommand.ControlResponse{}.Removal
				}
				return p, err
			}
			...
			_, err = requests.Cancel(t.Context(), "operator", f.scope, f.id, r.ID, r.Revision, uuid.NewString())
			require.NoError(t, err)
			for _, query := range []string{`DELETE FROM uem_netbird_removal_dispatch_stops`, `UPDATE uem_netbird_removal_dispatch_stops SET reason='expired'`} {
...
func TestNetbirdRemovalDispatchJoinsShutdownAndConcurrentDelivery(t *testing.T) {
	f, requests := removalFixture(t, nil)
	r := removalRequest(t, f, requests)
...
func TestNetbirdRemovalDispatchStopGuardsNativeAdmissionAndStartup(t *testing.T) {
	f, requests := removalFixture(t, nil)
	r := removalRequest(t, f, requests)
	var calls atomic.Int32
	s := removalDeliveryStore(t, f, nil, func(context.Context, netbirdcommand.Command) (*netbirdcommand.Receipt, error) {
		calls.Add(1)
		return nil, nil
	})
	_, err := f.db.ExecContext(t.Context(), `INSERT INTO uem_netbird_removal_dispatch_stops(request_id,reason) VALUES($1,'source_changed')`, r.ID)
	require.NoError(t, err)
	_, err = s.Uninstall(t.Context(), "tag-admin", f.scope, f.id, r.ID, r.Revision)
	require.ErrorIs(t, err, inventory.ErrNetbirdOperationConflict)
	require.Zero(t, calls.Load())
	_, err = f.db.ExecContext(t.Context(), `WITH stamp AS(SELECT clock_timestamp() AS at) INSERT INTO uem_netbird_removal_attempts(request_id,wire_version,certificate_hash,command_hash,issued_at,expires_at) SELECT $1,4,certificate_hash,repeat('f',64),stamp.at,stamp.at+interval '1 minute' FROM uem_agent_identities,stamp WHERE id=$2`, r.ID, f.id)
	...
	for _, guard := range [][2]string{{"uem_netbird_removal_dispatch_stops", "uem_netbird_removal_dispatch_stop_immutable"}, {"uem_netbird_removal_dispatch_stops", "uem_netbird_removal_dispatch_stop_valid"}, {"uem_netbird_removal_attempts", "uem_netbird_removal_dispatch_required"}} {
...
func TestNetbirdRemovalDispatchExpiresWithoutNativeInspection(t *testing.T) {
	f, requests := removalFixture(t, nil)
	r := removalRequest(t, f, requests)
	_, err := requests.Cancel(t.Context(), "operator", f.scope, f.id, r.ID, r.Revision, uuid.NewString())
	require.NoError(t, err)
	id := uuid.NewString()
	_, err = f.db.ExecContext(t.Context(), `WITH stamp AS(SELECT clock_timestamp() AS at) INSERT INTO uem_netbird_removals(id,device_id,tenant_id,site_id,actor,revision,journal_revision,descriptor_digest,descriptor,requested_at,expires_at)
 SELECT $2,device_id,tenant_id,site_id,actor,revision,journal_revision,descriptor_digest,descriptor,stamp.at-interval '20 minutes',stamp.at-interval '10 minutes' FROM uem_netbird_removals,stamp WHERE id=$1`, r.ID, id)
	require.NoError(t, err)
	var calls atomic.Int32
	s := removalDeliveryStore(t, f, func(context.Context, netbirdcommand.ControlRequest) (*netbirdcommand.ControlResponse, error) {
		calls.Add(1)
		return nil, nil
