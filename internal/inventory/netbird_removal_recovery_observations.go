...
// ObserveRecovery recovers retained receipts using current individual
...
func (s *NetbirdRemovalRecoveryStore) ObserveRecovery(parent context.Context, actor string, scope access.Scope, device, id, revision string) (*NetbirdRemovalRecoveryDelivery, error) {
	...
	v, _, err := operations.resolutionTarget(ctx, tx, &NetbirdOperation{ID: id, DeviceID: device, Scope: scope, Individual: true, Revision: r.Revision, Operation: "recover-removal", CommandHash: d.CommandHash})
	if err != nil {
		return nil, err
	}
	...
	if _, err = recordRecoveryObservation(ctx, tx, r, actor, query, response); err != nil {
		return nil, err
	}
	if err = removalRecoveryStageAudit(ctx, tx, r, actor, "resolution.observe", "receipt"); err != nil {
		return nil, err
	}
	d, err = readRemovalRecoveryDelivery(ctx, tx, id)
	if err != nil {
...
func recordRecoveryObservation(ctx context.Context, tx *sql.Tx, r *NetbirdRemovalRecovery, actor string, query netbirdcommand.ControlRequest, response *netbirdcommand.ControlResponse) (time.Time, error) {
	...
	err = tx.QueryRowContext(ctx, `INSERT INTO uem_netbird_removal_recovery_observations(id,request_id,actor,control_hash,control,response) VALUES($1,$2,$3,$4,$5::jsonb,$6::jsonb) RETURNING recorded_at`, query.RequestID, r.ID, actor, hash, string(control), data).Scan(&recorded)
	if err != nil {
		return time.Time{}, err
	}
	if completed {
		_, err = tx.ExecContext(ctx, `UPDATE uem_netbird_removal_recoveries SET completed_at=$2 WHERE id=$1 AND completed_at IS NULL AND released_at IS NULL`, r.ID, recorded)
	}
	return recorded, err
}
