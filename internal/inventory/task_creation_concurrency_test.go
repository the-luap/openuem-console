...
func TestTaskCreationHoldsParentSiblingsAndAuthorityThroughCommit(t *testing.T) {
	...
	require.NoError(t, err)
	_, err = conn.ExecContext(ctx, "SELECT pg_advisory_lock(673810073)")
	require.NoError(t, err)
	var once sync.Once
	release := func() {
		once.Do(func() {
			_, _ = conn.ExecContext(context.Background(), "SELECT pg_advisory_unlock(673810073)")
			_ = conn.Close()
		})
	}
	t.Cleanup(release)
	_, err = f.db.ExecContext(ctx, "CREATE SEQUENCE owned_task_creation_entered; CREATE FUNCTION hold_owned_task_creation() RETURNS trigger LANGUAGE plpgsql AS $$ BEGIN IF NEW.action='inventory.tasks.create' THEN PERFORM nextval('owned_task_creation_entered'); PERFORM pg_advisory_xact_lock(673810073); END IF; RETURN NEW; END $$; CREATE TRIGGER hold_owned_task_creation AFTER INSERT ON uem_inventory_audit FOR EACH ROW EXECUTE FUNCTION hold_owned_task_creation()")
	require.NoError(t, err)
	type outcome struct {
		id  int64
		err error
	...
	go func() {
		id, err := inventory.CreateLegacyTask(ctx, f.db, f.permissions, "tag-admin", f.scope, int64(g.profile), ownedTaskConfiguration(), "")
		done <- outcome{id, err}
	}()
	require.Eventually(t, func() bool {
		var entered bool
		return f.db.QueryRowContext(ctx, "SELECT is_called FROM owned_task_creation_entered").Scan(&entered) == nil && entered
	}, 2*time.Second, 10*time.Millisecond)
	current, err := f.client.Task.Get(ctx, g.task)
	require.NoError(t, err)
	require.Zero(t, current.Order, "uncommitted task normalization was published before audit")
	for _, mutation := range []func(context.Context) error{
		...
		func(ctx context.Context) error {
			_, err := inventory.CreateLegacyTask(ctx, f.db, f.permissions, "admin", f.scope, int64(g.profile), ownedTaskConfiguration(), "")
			return err
		},
		func(ctx context.Context) error {
	...
	case <-time.After(2 * time.Second):
		t.Fatal("task creation did not leave its audit gate")
	}
