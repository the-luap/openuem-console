...
func TestNetbirdRecoveryDispatchDeliversOnceAcrossRestart(t *testing.T) {
	...
	execute := func(ctx context.Context, c netbirdcommand.Command) (*netbirdcommand.Receipt, error) {
		...
		require.Nil(t, stop)
		var admitted int
		require.NoError(t, f.db.QueryRowContext(ctx, `SELECT count(*) FROM uem_netbird_removal_recovery_attempts WHERE request_id=$1`, r.ID).Scan(&admitted))
		require.Equal(t, 1, admitted)
		_, err = f.db.ExecContext(ctx, `INSERT INTO uem_netbird_removal_recovery_dispatch_stops(request_id,reason) VALUES($1,'source_changed')`, r.ID)
		require.ErrorContains(t, err, "cannot stop before delivery")
		out, err := netbirdcommand.ReceiptFor(c, "completed")
		return &out, err
	}
	s := recoveryDeliveryStore(t, x, nil, execute)
	...
	restarted := recoveryDeliveryStore(t, x, nil, execute)
...
func TestNetbirdRecoveryDispatchRetainsUncertainAttemptsWithoutRetry(t *testing.T) {
	for _, outcome := range []string{"lost", "rejected", "result-audit"} {
		t.Run(outcome, func(t *testing.T) {
			...
			if outcome == "result-audit" {
				_, err := f.db.ExecContext(t.Context(), `ALTER TABLE uem_netbird_operations_audit ADD CONSTRAINT owned_removal_dispatch_audit CHECK(resource_id NOT LIKE '%/recover-removal/result-unconfirmed') NOT VALID`)
				require.NoError(t, err)
			}
			worked, err := s.DispatchOne(t.Context())
			require.True(t, worked)
...
func TestNetbirdRecoveryDispatchStopsChangedPreflightWithoutClearingBarrier(t *testing.T) {
	for _, change := range []string{"permission", "certificate", "descriptor", "absent", "unavailable", "stop-audit"} {
		t.Run(change, func(t *testing.T) {
			...
			control := func(ctx context.Context, c netbirdcommand.ControlRequest) (*netbirdcommand.ControlResponse, error) {
				...
				p, err := recoveryControl(t, x)(ctx, c)
				if err == nil && change == "descriptor" {
					p.RemovalRecovery.StateDigest = strings.Repeat("e", 64)
				}
				if change == "absent" {
					p.Outcome = "absent"
					p.RemovalRecovery = netbirdcommand.RemovalRecovery{}
				}
				return p, err
			}
			...
			_, err = requests.Cancel(t.Context(), "operator", f.scope, f.id, r.ID, r.Revision, uuid.NewString())
			require.NoError(t, err)
			for _, query := range []string{`DELETE FROM uem_netbird_removal_recovery_dispatch_stops`, `UPDATE uem_netbird_removal_recovery_dispatch_stops SET reason='expired'`} {
...
func TestNetbirdRecoveryDispatchJoinsShutdownAndConcurrentDelivery(t *testing.T) {
	x, requests, r := recoveryDeliveryFixture(t)
	f := x.f
...
func TestNetbirdRecoveryDispatchStopGuardsNativeAdmissionAndStartup(t *testing.T) {
	...
	for _, guard := range [][2]string{{"uem_netbird_removal_recovery_dispatch_stops", "uem_netbird_removal_recovery_dispatch_stop_immutable"}, {"uem_netbird_removal_recovery_dispatch_stops", "uem_netbird_removal_recovery_dispatch_stop_valid"}, {"uem_netbird_removal_recovery_attempts", "uem_netbird_removal_recovery_dispatch_required"}} {
...
func TestNetbirdRecoveryDispatchExpiresWithoutNativeInspection(t *testing.T) {
	x, _, r := recoveryDeliveryFixture(t)
	f := x.f
	id := r.ID
	...
	require.NoError(t, err)
	_, err = f.db.ExecContext(t.Context(), `UPDATE uem_netbird_removal_recoveries SET expires_at=clock_timestamp() WHERE id=$1`, r.ID)
	require.NoError(t, err)
