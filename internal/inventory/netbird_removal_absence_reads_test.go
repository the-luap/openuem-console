...
func TestNetbirdAbsenceStatusAndHistoryRetainScopeWithoutLiveAuthority(t *testing.T) {
	var calls atomic.Int32
	x := absenceFixture(t)
	f := x.f
	requests := absenceStore(t, x, func(ctx context.Context, c netbirdcommand.ControlRequest) (*netbirdcommand.ControlResponse, error) {
		calls.Add(1)
		return absenceControl(t, x)(ctx, c)
	})
	var ids []string
	for range 22 {
		r := absenceRequest(t, x, requests, absenceReview(t, x, requests))
		ids = append(ids, r.ID)
		_, err := requests.Cancel(t.Context(), "operator", f.scope, f.id, r.ID, r.Revision, uuid.NewString())
		require.NoError(t, err)
	}
	last := absenceRequest(t, x, requests, absenceReview(t, x, requests))
	...
	for _, group := range [][]*inventory.NetbirdRemovalAbsenceStatus{page.Requests, older.Requests} {
		for _, r := range group {
...
func TestNetbirdAbsenceStatusCombinesReleasedAndOriginalUnconfirmedEvidence(t *testing.T) {
