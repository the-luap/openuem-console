...
type removalResolutionFixture struct {
	f                          *refreshFixture
	s, requests                *inventory.NetbirdRemovalStore
	nativeAbsent               bool
	inspections                int
	r                          *inventory.NetbirdRemoval
...
func TestNetbirdRemovalStartupRequiresAdmissionAndResultProtection(t *testing.T) {
	f, _ := removalFixture(t, nil)
	for _, guard := range [][2]string{
		{"uem_netbird_removals", "uem_netbird_removal_immutable"},
		{"uem_netbird_removals", "uem_netbird_removal_initial"},
		{"uem_netbird_removals", "uem_netbird_registration_admission"},
		{"uem_netbird_removals", "uem_netbird_removal_completion_valid"},
		{"uem_netbird_removal_attempts", "uem_netbird_removal_attempt_immutable"},
		{"uem_netbird_removal_attempts", "uem_netbird_removal_attempt_valid"},
		{"uem_netbird_removal_results", "uem_netbird_removal_result_immutable"},
		{"uem_netbird_removal_results", "uem_netbird_removal_result_valid"},
		{"uem_netbird_removal_observations", "uem_netbird_removal_observation_immutable"},
		{"uem_netbird_removal_observations", "uem_netbird_removal_observation_valid"},
	} {
		_, err := f.db.ExecContext(t.Context(), `ALTER TABLE `+guard[0]+` DISABLE TRIGGER `+guard[1])
		require.NoError(t, err)
		require.Error(t, inventory.Migrate(t.Context(), f.db), guard[1])
...
func newRemovalResolutionFixture(t *testing.T, status string) *removalResolutionFixture {
	t.Helper()
	f, requests := removalFixture(t, nil)
	x := &removalResolutionFixture{f: f, requests: requests, status: status, canRelease: true, journalRevision: strings.Repeat("a", 64)}
	x.r = removalRequest(t, f, requests)
	x.s = removalDeliveryStore(t, f, func(ctx context.Context, c netbirdcommand.ControlRequest) (*netbirdcommand.ControlResponse, error) {
		if c.Kind == "removal-state" {
			x.inspections++
			reply, err := removalControl(ctx, c)
			if err == nil && x.nativeAbsent {
				reply.Outcome = "absent"
				reply.Removal = packageapi.Removal{}
			}
			return reply, err
		}
		if c.Kind == "state" {
		...
		if outcome == "ok" {
			hash, e := x.command.Digest()
			require.NoError(t, e)
			p.Receipt = netbirdcommand.Receipt{Version: 1, RequestID: x.r.ID, DeviceID: f.id, Revision: x.r.Revision, CommandHash: hash, Operation: "uninstall", Status: x.status}
	...
	_, err := x.s.Uninstall(t.Context(), "tag-admin", f.scope, f.id, x.r.ID, x.r.Revision)
	require.NoError(t, err)
	return x
}

func (x *removalResolutionFixture) review(t *testing.T) *inventory.NetbirdRemovalResolutionReview {
	t.Helper()
	v, err := x.s.ReviewRemovalResolution(t.Context(), "operator", x.f.scope, x.f.id, x.r.ID, x.r.Revision)
	require.NoError(t, err)
	return v
}
func (x *removalResolutionFixture) resolve(t *testing.T, v *inventory.NetbirdRemovalResolutionReview) *inventory.NetbirdRemovalResolution {
	t.Helper()
	d, err := x.s.ResolveRemoval(t.Context(), "operator", x.f.scope, x.f.id, x.r.ID, x.r.Revision, v.ResolutionID, v.Revision)
	require.NoError(t, err)
	return d
}
func (x *removalResolutionFixture) reconcile(t *testing.T, id string) *inventory.NetbirdRemovalResolution {
	t.Helper()
	d, err := x.s.ReconcileRemovalResolution(t.Context(), "operator", x.f.scope, x.f.id, x.r.ID, x.r.Revision, id)
	require.NoError(t, err)
	return d
}

func TestNetbirdRemovalResolutionCommitsReviewedControlBeforeTransport(t *testing.T) {
	for _, status := range []string{"missing", "unconfirmed"} {
		t.Run(status, func(t *testing.T) {
			...
			x.beforeMutation = func(ctx context.Context, c netbirdcommand.ControlRequest) {
				...
				require.NoError(t, x.f.db.QueryRowContext(ctx, `SELECT control::text,control_hash FROM uem_netbird_removal_controls WHERE request_id=$1`, x.r.ID).Scan(&retained, &hash))
				...
				read, err := x.s.ReadRemovalResolution(ctx, "viewer", x.f.scope, x.f.id, x.r.ID, x.r.Revision)
				require.NoError(t, err)
			...
			// Exercise the removal partial unique index through normal admission.
			fresh, err := x.requests.Request(t.Context(), "tag-admin", x.f.scope, x.f.id, uuid.NewString(), x.r.DescriptorDigest, x.r.Revision)
			require.NoError(t, err)
...
func TestNetbirdRemovalResolutionRecoversLostReplyUnderCurrentAuthority(t *testing.T) {
	for _, status := range []string{"missing", "unconfirmed"} {
		t.Run(status, func(t *testing.T) {
			x := newRemovalResolutionFixture(t, status)
			v := x.review(t)
			...
			observed, err := x.s.ObserveRemoval(t.Context(), "operator", x.f.scope, x.f.id, x.r.ID, x.r.Revision)
			require.NoError(t, err)
			...
			var outcome string
			require.NoError(t, x.f.db.QueryRowContext(t.Context(), `SELECT outcome FROM uem_netbird_removal_results WHERE request_id=$1`, x.r.ID).Scan(&outcome))
			require.Equal(t, "unconfirmed", outcome)
		})
	}
}

func TestNetbirdRemovalResolutionRetriesOnlyNewReviewedControl(t *testing.T) {
...
func TestNetbirdRemovalResolutionRejectsChangedOrExpiredReview(t *testing.T) {
	for _, change := range []string{"certificate", "consumer", "permission", "scope", "revision", "journal", "receipt", "expiry", "audit"} {
		t.Run(change, func(t *testing.T) {
			...
			case "receipt":
				x.status = "unconfirmed"
			case "expiry":
				_, err = x.f.db.ExecContext(t.Context(), `ALTER TABLE uem_netbird_removal_reviews DISABLE TRIGGER uem_netbird_removal_review_immutable`)
				require.NoError(t, err)
				_, err = x.f.db.ExecContext(t.Context(), `UPDATE uem_netbird_removal_reviews SET created_at=created_at-interval '3 minutes',expires_at=expires_at-interval '3 minutes'`)
			case "audit":
			...
			require.NoError(t, err)
			_, err = x.s.ResolveRemoval(t.Context(), "operator", scope, x.f.id, x.r.ID, revision, v.ResolutionID, v.Revision)
			require.Error(t, err)
			require.Zero(t, x.mutations)
			var n int
			require.NoError(t, x.f.db.QueryRowContext(t.Context(), `SELECT count(*) FROM uem_netbird_removal_controls`).Scan(&n))
			require.Zero(t, n)
		})
	}
}

func TestNetbirdRemovalResolutionKeepsBarrierForUnownedOrIneligibleEvidence(t *testing.T) {
	for _, condition := range []string{"orphan", "foreign", "missing-after-release", "released-without-release-attempt", "wrong-operation", "wrong-hash", "wrong-query", "unavailable", "audit"} {
		t.Run(condition, func(t *testing.T) {
			...
			got, err := x.s.ReconcileRemovalResolution(t.Context(), "operator", x.f.scope, x.f.id, x.r.ID, x.r.Revision, d.ID)
			if condition == "audit" {
				require.Error(t, err)
			} else {
				require.NoError(t, err)
			...
			_, err = x.f.db.ExecContext(t.Context(), `UPDATE uem_netbird_removals SET released_at=clock_timestamp(),released_by='operator',resolution_id=$2 WHERE id=$1`, x.r.ID, d.ID)
			require.Error(t, err)
			require.Equal(t, 1, x.mutations)
		})
	}
...
func TestNetbirdRemovalResolutionRecoversCompletionAndAtomicAuditFailure(t *testing.T) {
	for _, completion := range []bool{false, true} {
		t.Run(map[bool]string{false: "release", true: "completion"}[completion], func(t *testing.T) {
			...
			if completion {
				...
				require.Nil(t, d)
				var n int
				require.NoError(t, x.f.db.QueryRowContext(t.Context(), `SELECT count(*) FROM uem_netbird_removal_control_results`).Scan(&n))
				require.Zero(t, n)
...
func TestNetbirdRemovalResolutionIndependentSQLProofAndStartupGuards(t *testing.T) {
	x := newRemovalResolutionFixture(t, "unconfirmed")
	v := x.review(t)
	...
	require.NoError(t, err)
	_, err = x.s.ResolveRemoval(t.Context(), "operator", x.f.scope, x.f.id, x.r.ID, x.r.Revision, v.ResolutionID, v.Revision)
	require.Error(t, err)
	_, err = x.f.db.ExecContext(t.Context(), `ALTER TABLE uem_netbird_operations_audit DROP CONSTRAINT owned_removal_proof_audit`)
	require.NoError(t, err)
	var wire []byte
	var attempt string
	require.NoError(t, x.f.db.QueryRowContext(t.Context(), `SELECT id::text,control FROM uem_netbird_removal_controls WHERE request_id=$1`, x.r.ID).Scan(&attempt, &wire))
	c, err := netbirdcommand.DecodeControl(wire)
	require.NoError(t, err)
	p, err := netbirdcommand.ControlResponseFor(c, "ok")
	require.NoError(t, err)
	p.Receipt = netbirdcommand.Receipt{Version: 1, RequestID: x.r.ID, DeviceID: x.f.id, Revision: x.r.Revision, CommandHash: c.CommandHash, Operation: "uninstall", Status: "unconfirmed"}
	...
	for _, change := range []string{"status", "release", "revision", "certificate", "hash", "kind", "extra", "state", "missing-release"} {
		...
		require.NoError(t, e)
		_, err = x.f.db.ExecContext(t.Context(), `INSERT INTO uem_netbird_removal_control_results(attempt_id,response) VALUES($1,$2::jsonb)`, attempt, string(altered))
		require.Error(t, err, change)
	}
	var n int
	require.NoError(t, x.f.db.QueryRowContext(t.Context(), `SELECT count(*) FROM uem_netbird_removal_control_results`).Scan(&n))
	require.Zero(t, n)
	_, err = x.f.db.ExecContext(t.Context(), `INSERT INTO uem_netbird_removal_control_results(attempt_id,response) VALUES($1,$2::jsonb)`, attempt, string(encoded))
	require.NoError(t, err)
	for _, resolution := range []string{uuid.NewString(), x.r.ID} {
		_, err = x.f.db.ExecContext(t.Context(), `INSERT INTO uem_netbird_removal_release_proofs(request_id,resolution_id,actor,attempt_id) VALUES($1,$2,'operator',$3)`, x.r.ID, resolution, attempt)
		require.Error(t, err)
	}
	...
	require.NoError(t, x.f.db.QueryRowContext(t.Context(), `INSERT INTO uem_netbird_removal_release_proofs(request_id,resolution_id,actor,attempt_id) VALUES($1,$2,'operator',$3) RETURNING recorded_at`, x.r.ID, v.ResolutionID, attempt).Scan(&recorded))
	_, err = x.f.db.ExecContext(t.Context(), `UPDATE uem_netbird_removals SET released_at=$2,released_by='viewer',resolution_id=$3 WHERE id=$1`, x.r.ID, recorded, v.ResolutionID)
	require.Error(t, err)
	_, err = x.f.db.ExecContext(t.Context(), `UPDATE uem_netbird_removals SET released_at=$2,released_by='operator',resolution_id=$3 WHERE id=$1`, x.r.ID, recorded, v.ResolutionID)
	require.NoError(t, err)
	for _, pair := range [][2]string{{"reviews", "review"}, {"resolutions", "resolution"}, {"controls", "control"}, {"control_results", "control_result"}, {"release_proofs", "release_proof"}} {
		table := "uem_netbird_removal_" + pair[0]
		...
		for _, suffix := range []string{"immutable", "valid"} {
			guard := "uem_netbird_removal_" + pair[1] + "_" + suffix
	...
	for _, q := range []string{`UPDATE uem_netbird_removals SET released_at=NULL,released_by=NULL,resolution_id=NULL`, `UPDATE uem_netbird_removals SET completed_at=clock_timestamp()`} {
		_, err = x.f.db.ExecContext(t.Context(), q)
		require.Error(t, err)
	}
	_, err = x.f.db.ExecContext(t.Context(), `ALTER TABLE uem_netbird_removals DISABLE TRIGGER uem_netbird_removal_release_required`)
	require.NoError(t, err)
	require.Error(t, inventory.Migrate(t.Context(), x.f.db))
	_, err = x.f.db.ExecContext(t.Context(), `ALTER TABLE uem_netbird_removals ENABLE TRIGGER uem_netbird_removal_release_required`)
	require.NoError(t, err)
	require.NoError(t, inventory.Migrate(t.Context(), x.f.db))
}

func TestNetbirdRemovalResolutionIndependentSQLControlAdmission(t *testing.T) {
	x := newRemovalResolutionFixture(t, "missing")
	v := x.review(t)
	var reviewID string
	require.NoError(t, x.f.db.QueryRowContext(t.Context(), `SELECT id::text FROM uem_netbird_removal_reviews WHERE revision=$1`, v.Revision).Scan(&reviewID))
	_, err := x.f.db.ExecContext(t.Context(), `INSERT INTO uem_netbird_removal_resolutions(request_id,id,actor,review_id) VALUES($1,$2,'operator',$3)`, x.r.ID, v.ResolutionID, reviewID)
	require.NoError(t, err)
	var issued time.Time
	require.NoError(t, x.f.db.QueryRowContext(t.Context(), `SELECT clock_timestamp()`).Scan(&issued))
	hash, err := x.command.Digest()
	require.NoError(t, err)
	c := netbirdcommand.ControlRequest{Version: 2, Identity: x.command.Identity, RequestID: v.ResolutionID, Kind: "withdraw", ReferenceID: x.r.ID, CommandHash: hash, Revision: x.r.Revision, Operation: "uninstall", IssuedAt: issued, ExpiresAt: issued.Add(5 * time.Second)}
	...
	require.NoError(t, e)
	response.Receipt = netbirdcommand.Receipt{Version: 1, RequestID: x.r.ID, DeviceID: x.f.id, Revision: x.r.Revision, CommandHash: hash, Operation: "uninstall", Status: "withdrawn"}
	response.ReleaseID = v.ResolutionID
	proof, e := netbirdcommand.EncodeControlResponse(query, response)
	require.NoError(t, e)
	_, err = x.f.db.ExecContext(t.Context(), `INSERT INTO uem_netbird_removal_observations(id,request_id,actor,control_hash,control,response) VALUES($1,$2,'operator',$3,$4::jsonb,$5::jsonb)`, query.RequestID, x.r.ID, qhash, string(qwire), string(proof))
	require.NoError(t, err)
	insert := `INSERT INTO uem_netbird_removal_controls(id,request_id,resolution_id,review_id,actor,sequence,kind,control_hash,control) VALUES($1,$2,$3,$4,'operator',1,'withdraw',$5,$6::jsonb)`
	...
	require.NoError(t, err)
	_, err = x.f.db.ExecContext(t.Context(), `INSERT INTO uem_netbird_removal_release_proofs(request_id,resolution_id,actor,observation_id) VALUES($1,$2,'operator',$3)`, x.r.ID, v.ResolutionID, query.RequestID)
	require.Error(t, err)
	require.Zero(t, x.mutations)
}
