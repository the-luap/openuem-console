...
func TestAppleProfileGroupPages(t *testing.T) {
	...
	for _, state := range []string{"choose", "empty", "preview", "remove", "excluded", "assignment", "history", "empty-history", "long"} {
		t.Run(state, func(t *testing.T) {
			...
			if state == "preview" || state == "remove" || state == "long" {
