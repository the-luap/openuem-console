...
func TestManagementPagesRenderSafeFormsAndInventory(t *testing.T) {
	...
	cases := []struct {
		...
		{"mac-linked-reader", DeviceDetails(c, &reader, linked), []string{"Configured update policy", "Target build: 24B1", "Reported build: 24A335", "Device identity", "Management channel history", "Open agent inventory", linked.Mac.ID}},
		...
		{"apple-profile-group-catalog", Profiles(c, &sourceSite, []apple.Profile{p}, []apple.Device{*d}), []string{"data-profile-group-actions", "Apply to a dynamic group", "Remove from a dynamic group", "Group assignment history"}},
		{"profiles", Profiles(c, info, []apple.Profile{p}, []apple.Device{*d}), []string{"Create a Wi-Fi profile", "Create a Mac firewall profile", "Create a Mac Platform SSO profile", "Create a Mac Gatekeeper profile", "Create a Mac System Extensions profile", "Create a Mac privacy profile (PPPC/TCC)", "Receiver code requirement", "Create a PKCS12 identity profile", "The target device checks the password", "Create a public certificate profile", "Create a SCEP certificate profile", "Pending-response retries", "Public certificates (PEM or DER, up to 256 KiB)", "Prevent removal from System Settings and Finder", "Blocked malware submission prompt", "Deleting this catalog entry retains its encrypted revision history.", "Provider registration token", "Allowed applications", "Save and deploy revision", "Company Wi-Fi", "test-csrf-token"}},
	...
	for _, state := range []string{"first", "next", "empty", "long"} {
		t.Run("device-list-"+state, func(t *testing.T) {
			rows := []DeviceRow{}
			for i := range 25 {
			...
			if state == "long" {
				rows[0].Name = strings.Repeat("Owned long device ", 20) + "<script>"
			...
			if dir := os.Getenv("APPLE_MDM_UI_ARTIFACTS"); dir != "" {
				...
				if err := os.WriteFile(filepath.Join(dir, "device-list-"+state+".html"), rendered.Bytes(), 0644); err != nil {
					t.Fatal(err)
				}
			}
		})
