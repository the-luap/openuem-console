...
func TestManagementPagesRenderSafeFormsAndInventory(t *testing.T) {
	...
	cases := []struct {
		...
		{"profiles", Profiles(c, info, []apple.Profile{p}, []apple.Device{*d}), []string{"Create a Wi-Fi profile", "Create a Mac firewall profile", "Create a Mac Platform SSO profile", "Provider registration token", "Allowed applications", "Save and deploy revision", "Company Wi-Fi", "test-csrf-token"}},
