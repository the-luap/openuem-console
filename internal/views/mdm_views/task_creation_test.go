...
func TestTaskCreationForms(t *testing.T) {
	...
	for state, info := range map[string]*partials.CommonInfo{"global": {TenantID: "-1", SiteID: "-1"}, "organization": {TenantID: "1", SiteID: "-1"}, "site": {TenantID: "1", SiteID: "2"}, "long": {TenantID: "1", SiteID: "2"}} {
		...
		base := partials.GetNavigationUrl(info, "/tasks/17/new")
		...
		require.NoError(t, partials.SelectTaskType(nil, "windows", base).Render(ctx, &body))
		body.WriteString(`</template><template id="owned-linux-types">`)
		require.NoError(t, partials.SelectTaskType(nil, "linux", base).Render(ctx, &body))
		...
		require.NoError(t, partials.SelectRegistryTaskSubtype(nil, base).Render(ctx, &body))
		body.WriteString(`</template><template id="owned-any-types">`)
		require.NoError(t, partials.SelectTaskType(nil, "any", base).Render(ctx, &body))
		body.WriteString(`</template><template id="owned-netbird-subtypes">`)
		require.NoError(t, partials.SelectNetbirdTaskSubtype(nil, base).Render(ctx, &body))
