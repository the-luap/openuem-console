...
func TestOrganizationTagPages(t *testing.T) {
	...
	for _, state := range []string{"list", "empty", "detail", "used", "viewer", "long", "list-viewer", "administrator", "legacy-palette"} {
		t.Run(state, func(t *testing.T) {
			...
			require.Equal(t, state == "detail" || state == "long" || state == "administrator" || state == "legacy-palette", strings.Contains(html, `aria-label="Delete tag"`))
