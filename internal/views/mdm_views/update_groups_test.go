...
func TestAppleUpdateGroupPages(t *testing.T) {
	...
	for _, state := range []string{"choose", "empty", "preview", "existing", "excluded", "assignment", "history", "empty-history", "long"} {
		t.Run(state, func(t *testing.T) {
			...
			assignment := apple.UpdatePlanGroupAssignment{ID: "40000000-0000-0000-0000-000000000001", Scope: plan.Scope, RequestKey: "50000000-0000-0000-0000-000000000001", Plan: plan, Actor: "owned-operator", Group: apple.ProfileGroupSource{ID: group.ID, Revision: group.Revision, Name: group.Name, Rule: group.Rule}, CreatedAt: now, Commands: []apple.UpdatePlanGroupCommand{{Selection: target.Selection, CommandID: "60000000-0000-0000-0000-000000000001"}}}
			...
			case "choose", "empty":
				...
				component = AppleUpdatePlanGroups(c, info, plan, &inventory.DeviceGroupPage{Groups: groups}, DevicePagination{Next: UpdatePlanGroupChoiceURL(info, plan, group.ID)})
			case "assignment":
			...
			if dir := os.Getenv("APPLE_MDM_UI_ARTIFACTS"); dir != "" {
				...
				require.NoError(t, os.WriteFile(filepath.Join(dir, "apple-update-group-"+state+".html"), body.Bytes(), 0644))
			}
		})
	}
}
