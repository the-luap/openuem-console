...
func TestNetbirdRemovalStageCleanupViews(t *testing.T) {
	...
	for _, kind := range kinds {
		t.Run(kind, func(t *testing.T) {
			...
			r := &inventory.NetbirdRemovalStageCleanup{ID: "10000000-0000-4000-8000-000000000001", DeviceID: target.ID, Scope: scope, Actor: "Owned <operator>", OriginalID: "60000000-0000-4000-8000-000000000006", StageCleanup: netbirdcommand.RemovalStageCleanup{Original: netbirdcommand.RemovalStageCleanupReference{RequestID: "60000000-0000-4000-8000-000000000006", ReleaseID: "70000000-0000-4000-8000-000000000007", CommandHash: strings.Repeat("f", 64), Revision: strings.Repeat("a", 64)}, Profile: netbirdcommand.RemovalStageCleanupProfile, DirectoryCount: 7, ManifestPresent: true, ManifestBytes: 128, JournalRevision: strings.Repeat("e", 64), StateDigest: strings.Repeat("f", 64)}, StageCleanupDigest: strings.Repeat("b", 64), Revision: strings.Repeat("c", 64), RequestedAt: instant, ExpiresAt: instant.Add(2 * time.Minute)}
			if kind == "review-no-manifest" {
				r.StageCleanup.ManifestPresent = false
				r.StageCleanup.ManifestBytes = 0
				r.StageCleanup.DirectoryCount = 1
			}
			if kind == "review-max" {
				r.StageCleanup.ManifestBytes = 2 << 20
			}
			status := &inventory.NetbirdRemovalStageCleanupStatus{Request: r}
			resolution := &inventory.NetbirdRemovalStageCleanupResolution{ID: "20000000-0000-4000-8000-000000000002", RequestID: r.ID, Actor: r.Actor, Kind: "withdraw", CreatedAt: instant, LastAttempt: &inventory.NetbirdRemovalStageCleanupControlAttempt{ID: "40000000-0000-4000-8000-000000000004", Actor: r.Actor, Kind: "withdraw", Sequence: 1, CreatedAt: instant, Outcome: "unavailable"}}
			var component templ.Component
			switch {
			case strings.HasPrefix(kind, "review"):
				component = NetbirdRemovalStageCleanupReview(c, info, &inventory.NetbirdRemovalStageCleanupReview{Target: target, StageCleanup: r.StageCleanup, StageCleanupDigest: r.StageCleanupDigest, Revision: r.Revision}, r.ID)
			case strings.HasPrefix(kind, "history"):
				page := &inventory.NetbirdRemovalStageCleanupHistory{Requests: []*inventory.NetbirdRemovalStageCleanupStatus{status}}
				...
				component = NetbirdRemovalStageCleanupHistory(c, info, scope, target.ID, page)
			case strings.HasPrefix(kind, "resolution"):
				expiry := instant.Add(2 * time.Minute)
				v := &inventory.NetbirdRemovalStageCleanupResolutionReview{RequestID: r.ID, ResolutionID: resolution.ID, Revision: strings.Repeat("d", 64), ExpiresAt: &expiry, Kind: "withdraw", Outcome: "not-received"}
				switch kind {
				...
				component = NetbirdRemovalStageCleanupResolution(c, info, status, v)
			default:
				if kind == "delivery-pending" || kind == "unconfirmed" || kind == "viewer" || kind == "completed" || kind == "released" {
					status.Delivery = &inventory.NetbirdRemovalStageCleanupDelivery{RequestID: r.ID, CommandHash: strings.Repeat("e", 64), IssuedAt: instant, ExpiresAt: instant.Add(5 * time.Minute), Outcome: "unconfirmed", OriginalOutcome: "unconfirmed"}
				...
				if kind == "stopped" {
					status.DispatchStop = &inventory.NetbirdRemovalStageCleanupDispatchStop{RequestID: r.ID, Reason: "source_changed", RecordedAt: instant}
				}
				component = NetbirdRemovalStageCleanupReceipt(c, info, status, "50000000-0000-4000-8000-000000000005")
			}
			...
			if strings.HasPrefix(kind, "review") {
				require.Contains(t, text, "This action removes the reviewed empty directories and any listed incomplete manifest metadata.")
			...
			if kind == "review-no-manifest" {
				...
				require.NotContains(t, text, "128 bytes")
			}
			if kind == "review-max" {
				require.Contains(t, text, "2097152 bytes")
			}
			if kind == "viewer" {
				require.NotContains(t, actions, "<form")
				require.NotContains(t, text, "Review cleanup resolution")
			}
			...
			if dir := os.Getenv("APPLE_MDM_UI_ARTIFACTS"); dir != "" {
				...
				require.NoError(t, os.WriteFile(filepath.Join(dir, "netbird-removal-stage-cleanups-"+kind+".html"), body.Bytes(), 0600))
			}
		})
	}
}
