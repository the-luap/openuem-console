...
func TestNetbirdRemovalViews(t *testing.T) {
	...
	for _, kind := range kinds {
		t.Run(kind, func(t *testing.T) {
			...
			r := &inventory.NetbirdRemoval{ID: "10000000-0000-4000-8000-000000000001", DeviceID: target.ID, Scope: scope, Actor: "Owned <operator>", Descriptor: pkg, DescriptorDigest: strings.Repeat("b", 64), Revision: strings.Repeat("c", 64), RequestedAt: instant, ExpiresAt: instant.Add(10 * time.Minute)}
			status := &inventory.NetbirdRemovalStatus{Request: r}
			resolution := &inventory.NetbirdRemovalResolution{ID: "20000000-0000-4000-8000-000000000002", RequestID: r.ID, Actor: r.Actor, Kind: "withdraw", CreatedAt: instant, LastAttempt: &inventory.NetbirdRemovalControlAttempt{ID: "40000000-0000-4000-8000-000000000004", Actor: r.Actor, Kind: "withdraw", Sequence: 1, CreatedAt: instant, Outcome: "unavailable"}}
			var component templ.Component
			switch {
			case strings.HasPrefix(kind, "review"):
				component = NetbirdRemovalReview(c, info, &inventory.NetbirdRemovalReview{Target: target, Descriptor: pkg, DescriptorDigest: r.DescriptorDigest, Revision: r.Revision, Absent: kind == "review-absent"}, r.ID)
			case strings.HasPrefix(kind, "history"):
				page := &inventory.NetbirdRemovalHistory{Requests: []*inventory.NetbirdRemovalStatus{status}}
				...
				component = NetbirdRemovalHistory(c, info, scope, target.ID, page)
			case strings.HasPrefix(kind, "resolution"):
				expiry := instant.Add(2 * time.Minute)
				v := &inventory.NetbirdRemovalResolutionReview{RequestID: r.ID, ResolutionID: resolution.ID, Revision: strings.Repeat("d", 64), ExpiresAt: &expiry, Kind: "withdraw", Outcome: "not-received"}
				switch kind {
				...
				component = NetbirdRemovalResolution(c, info, status, v)
			default:
				if kind == "delivery-pending" || kind == "unconfirmed" || kind == "viewer" || kind == "completed" || kind == "released" {
					status.Delivery = &inventory.NetbirdRemovalDelivery{RequestID: r.ID, CommandHash: strings.Repeat("e", 64), IssuedAt: instant, ExpiresAt: instant.Add(10 * time.Minute), Outcome: "unconfirmed", OriginalOutcome: "unconfirmed"}
				...
				if kind == "stopped" {
					status.DispatchStop = &inventory.NetbirdRemovalDispatchStop{RequestID: r.ID, Reason: "source_changed", RecordedAt: instant}
				}
				component = NetbirdRemovalReceipt(c, info, status, "50000000-0000-4000-8000-000000000005")
			}
			...
			if kind == "viewer" {
				...
				require.NotContains(t, text, "Review removal recovery")
			}
			...
			if dir := os.Getenv("APPLE_MDM_UI_ARTIFACTS"); dir != "" {
				...
				require.NoError(t, os.WriteFile(filepath.Join(dir, "netbird-removals-"+kind+".html"), body.Bytes(), 0600))
			}
		})
	}
}
