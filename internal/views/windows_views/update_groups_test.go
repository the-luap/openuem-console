...
func TestWindowsGroupAssignmentViews(t *testing.T) {
	...
	for _, state := range []string{"choose", "empty", "form", "preview", "history", "long"} {
		t.Run(state, func(t *testing.T) {
			...
			case "choose", "empty":
				...
				view = UpdateAssignmentGroups(c, info, ring, "apply", &inventory.DeviceGroupPage{Groups: groups}, mdm_views.DevicePagination{Next: "/tenant/1/site/1/windows/update-rings/" + ring.RingID + "/assign/groups?revision=1&mode=apply&after=" + group.ID})
			case "form":
			...
			if state == "form" || state == "preview" || state == "long" {
