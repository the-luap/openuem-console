...
ALTER TABLE uem_inventory_audit ADD CONSTRAINT uem_inventory_audit_action_check
 CHECK (action IN ('inventory.desktop.read','inventory.software.read','inventory.network.read','inventory.storage.read','inventory.peripherals.read','inventory.memory.read','inventory.shares.read','inventory.security.read','inventory.devices.list','inventory.devices.export_csv','inventory.devices.export_json','inventory.groups.list','inventory.groups.read','inventory.groups.create','inventory.groups.revise'));
...
ALTER TABLE uem_inventory_audit ADD CONSTRAINT uem_inventory_audit_site_id_check
 CHECK (site_id>0 OR (site_id=0 AND action IN ('inventory.devices.list','inventory.devices.export_csv','inventory.devices.export_json','inventory.groups.list','inventory.groups.read','inventory.groups.create','inventory.groups.revise')));
