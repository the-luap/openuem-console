...
var sourceQueries = append([]auditSource{
	...
	{"settings", "uem_settings_audit", `SELECT id,tenant_id,site_id,actor,action,resource_id AS resource,result,created_at FROM uem_settings_audit`},
	...
	{"netbird-operations", "uem_netbird_operations_audit", `SELECT id,tenant_id,site_id,actor,action,resource_id AS resource,result,created_at FROM uem_netbird_operations_audit`},
