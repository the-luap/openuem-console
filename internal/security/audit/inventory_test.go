...
func TestInventoryAuditScopeExportAndRetentionWithoutEnrollment(t *testing.T) {
 ...
 (2,21,'reader','inventory.desktop.read','legacy-foreign',clock_timestamp()-interval '45 days')`); err != nil {
	...
	var events []Event
	if err = json.Unmarshal(data, &events); err != nil || len(events) != 1 || events[0].Resource != "legacy-old" || events[0].SiteID != 11 {
	...
	if err != nil || preview.Counts["inventory"] != 1 {
	...
	if err = s.db.QueryRow(`SELECT count(*) FROM uem_inventory_audit WHERE resource_id='legacy-old'`).Scan(&remaining); err != nil || remaining != 0 {
		t.Fatal("confirmed inventory retention was not applied", remaining, err)
	}
	if err = s.db.QueryRow(`SELECT sum(event_count) FROM uem_audit_retention_history WHERE tenant_id=1 AND source='inventory'`).Scan(&history); err != nil || history != 1 {
