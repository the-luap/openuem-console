...
func TestInventoryAuditScopeExportAndRetentionWithoutEnrollment(t *testing.T) {
 ...
 (2,21,'reader','inventory.desktop.read','legacy-foreign',clock_timestamp()-interval '45 days')`); err != nil {
	...
	var events []Event
	if err = json.Unmarshal(data, &events); err != nil || len(events) != 6 {
		t.Fatal("inventory export lost historical scope", string(data), err)
	}
	want := map[string]string{"memory-old": "inventory.memory.read", "peripherals-old": "inventory.peripherals.read", "storage-old": "inventory.storage.read", "legacy-old": "inventory.desktop.read", "network-old": "inventory.network.read", "software-old": "inventory.software.read"}
	...
	if err != nil || preview.Counts["inventory"] != 6 {
	...
	if err = s.db.QueryRow(`SELECT count(*) FROM uem_inventory_audit WHERE resource_id IN ('memory-old','peripherals-old','storage-old','legacy-old','network-old','software-old')`).Scan(&remaining); err != nil || remaining != 0 {
		t.Fatal("confirmed inventory retention was not applied", remaining, err)
	}
	if err = s.db.QueryRow(`SELECT sum(event_count) FROM uem_audit_retention_history WHERE tenant_id=1 AND source='inventory'`).Scan(&history); err != nil || history != 6 {
