...
func TestRolesEnforceScopeAndCapabilities(t *testing.T) {
	...
	for _, role := range []Role{Viewer, Operator, TenantAdmin, Administrator} {
		...
		for _, scope := range scopes {
			...
			for _, capability := range []Capability{ReadDevices, ReadProfiles, EnrollDevices, RefreshDevices, AssignProfiles, ManageUpdates, ManageProfiles, ManageCertificates, RevokeDevices, ManageAccess, ReadAudit} {
				want := scopeAllowed
				switch capability {
				case ManageAccess:
					want = role == Administrator
				case ManageProfiles, ManageCertificates, RevokeDevices, ReadAudit:
