...
func TestRolesEnforceScopeAndCapabilities(t *testing.T) {
	...
	for _, role := range []Role{Viewer, Operator, TenantAdmin, Administrator} {
		...
		for _, scope := range scopes {
			...
			for _, capability := range []Capability{ManageDeviceGroups, ReadDevices, ReadProfiles, ReadSoftware, ManageSoftware, AssignSoftware, EnrollDevices, RefreshDevices, AssignProfiles, ManageUpdates, ManageProfiles, ManageCertificates, RevokeDevices, ManageAccess, ReadAudit, ManageAuditRetention, ManageDeviceSecurity, ManageWindowsCSP, RetrieveRecoveryKeys} {
				want := scopeAllowed
				switch capability {
				case ManageAccess:
