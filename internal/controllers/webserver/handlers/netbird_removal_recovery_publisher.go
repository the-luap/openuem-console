...
// PublishNetbirdRemovalRecovery accepts only a separate reviewed continuation.
// Its caller must commit the exact recovery delivery attempt before this RPC.
// A lost response requires receipt observation, never automatic redelivery.
func (h *Handler) PublishNetbirdRemovalRecovery(parent context.Context, c netbirdcommand.Command) (*netbirdcommand.Receipt, error) {
	if parent == nil || c.Version != netbirdcommand.RemovalRecoveryVersion || !c.Executable(c.Identity, time.Now()) {
