...
func exerciseNetbirdRemovalRoutes(t *testing.T, h *Handler, ctx context.Context, tenant, site, foreign int) {
	t.Helper()
	old, oldOps := h.NetbirdRemovals, h.NetbirdOperations
	defer func() { h.NetbirdRemovals, h.NetbirdOperations = old, oldOps }()
	e := echo.New()
	...
	require.NoError(t, h.Model.Client.Agent.Create().SetID(device).SetHostname("Owned removal <endpoint>").SetOs("macOS").SetAgentStatus(agent.AgentStatusEnabled).AddSiteIDs(site).Exec(ctx))
	...
	require.NoError(t, err)
	descriptor := packageapi.Removal{Schema: 1, Platform: "macos", Architecture: "arm64", Format: "pkg", PackageID: "io.netbird.client", Version: "0.78.1", StateDigest: strings.Repeat("a", 64)}
	absent := false
	...
	control := func(_ context.Context, c netbirdcommand.ControlRequest) (*netbirdcommand.ControlResponse, error) {
		if c.Kind == "removal-state" || c.Kind == "state" {
			...
			if c.Kind == "removal-state" {
				inspections++
				p.Removal = descriptor
				if absent {
					p.Outcome = "absent"
					p.Removal = packageapi.Removal{}
				}
			}
	...
	store, err := inventory.NewNetbirdRemovalDeliveryStore(h.Model.DB, h.Access, true, control, func(_ context.Context, c netbirdcommand.Command) (*netbirdcommand.Receipt, error) {
		native++
		command = c
	...
	path := fmt.Sprintf("/tenant/%d/site/%d/computers/%s/netbird/removals", tenant, site, device)
	...
	require.Equal(t, 403, request("scoped-viewer", "GET", path+"/review", nil, "").Code)
	absent = true
	absentReview := request("scoped-operator", "GET", path+"/review", nil, "")
	require.Equal(t, 200, absentReview.Code)
	require.Contains(t, absentReview.Body.String(), "No removal request is needed")
	require.NotContains(t, absentReview.Body.String(), "Queue removal")
	absent = false
	reviewPath := path + "/review"
	...
	for _, change := range []string{"confirmation", "duplicate", "unknown", "revision", "query"} {
		...
		case "query":
			target += "?descriptor_digest=" + strings.Repeat("a", 64)
		}
	...
	require.Equal(t, 200, response.Code, response.Body.String())
	require.NotContains(t, response.Body.String(), "Cancel removal request")
	require.Contains(t, response.Body.String(), "Queued for removal")
	...
	t.Run("reviewed native continuation lifecycle", func(t *testing.T) {
		exerciseNetbirdRemovalRecoveryRoutes(t, h, e, ctx, tenant, site, foreign, command, releaseID)
	})
	t.Run("reviewed current absence lifecycle", func(t *testing.T) {
		exerciseNetbirdRemovalAbsenceRoutes(t, h, e, ctx, tenant, site, foreign, command, releaseID)
	})
	t.Run("reviewed retained staging cleanup lifecycle", func(t *testing.T) {
		exerciseNetbirdRemovalStageCleanupRoutes(t, h, e, ctx, tenant, site, foreign, command, releaseID)
	})
