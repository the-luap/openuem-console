...
func exerciseNetbirdRemovalAbsenceRoutes(t *testing.T, h *Handler, e *echo.Echo, ctx context.Context, tenant, site, foreign int, original netbirdcommand.Command, originalRelease string) {
	t.Helper()
	old := h.NetbirdRemovalAbsences
	defer func() { h.NetbirdRemovalAbsences = old }()
	originalHash, err := original.Digest()
	require.NoError(t, err)
	reference := netbirdcommand.RemovalAbsenceReference{RequestID: original.RequestID, CommandHash: originalHash, Revision: original.Revision, ReleaseID: originalRelease}
	unavailable := false
	...
	control := func(_ context.Context, c netbirdcommand.ControlRequest) (*netbirdcommand.ControlResponse, error) {
		if c.Kind == "removal-absence-state" {
			inspections++
			require.Equal(t, reference, c.RemovalAbsenceOriginal)
			if unavailable {
			...
			p.RemovalAbsence = netbirdcommand.RemovalAbsence{Original: reference, Profile: netbirdcommand.RemovalAbsenceProfile, JournalRevision: p.State.Revision, StateDigest: strings.Repeat("a", 64)}
			return &p, err
		}
		if c.Kind == "state" {
			p, err := netbirdcommand.ControlResponseFor(c, "ok")
	...
	store, err := inventory.NewNetbirdRemovalAbsenceDeliveryStore(h.Model.DB, h.Access, true, control, func(_ context.Context, c netbirdcommand.Command) (*netbirdcommand.Receipt, error) {
		native++
		require.Equal(t, netbirdcommand.RemovalAbsenceVersion, c.Version)
		require.Equal(t, "verify-removal-absence", c.Operation)
		require.Equal(t, reference, c.RemovalAbsence.Original)
		command = c
		if receiptState != "completed" {
			return nil, errors.New("owned lost verification reply")
		}
		out, err := netbirdcommand.ReceiptFor(c, "completed")
		return &out, err
	})
	require.NoError(t, err)
	h.NetbirdRemovalAbsences = store
	request := ownedTagHTTPRequest(t, h, e, ctx)
	path := fmt.Sprintf("/tenant/%d/site/%d/computers/%s/netbird/removal-absences", tenant, site, original.DeviceID)
	...
	require.NotContains(t, unavailableReview.Body.String(), "Queue absence check")
	unavailable = false
	...
	for _, change := range []string{"confirmation", "duplicate", "unknown", "revision", "query", "original"} {
		...
		case "query":
			target += "?absence_digest=" + strings.Repeat("a", 64)
		}
	...
	require.Equal(t, 200, response.Code, response.Body.String())
	require.NotContains(t, response.Body.String(), "Cancel verification request")
	require.Contains(t, response.Body.String(), "Queued for removal verification")
	...
	for _, scenario := range []string{"completed", "observe", "withdraw", "release"} {
		...
		if scenario == "observe" {
			...
			absence := request("scoped-operator", "GET", receiptPath+"/resolution", nil, "")
			require.Equal(t, 200, absence.Code)
			resolved := fields(absence.Body.String())
		...
		if scenario == "completed" || scenario == "observe" {
			require.Contains(t, response.Body.String(), "Current package absence verified")
		} else {
	...
	require.NotContains(t, request("scoped-viewer", "GET", originalPath, nil, "").Body.String(), "Review current package absence")
	var outcome string
