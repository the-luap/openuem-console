...
// PublishNetbirdRemovalAbsence accepts only a separate reviewed current-absence verification.
// Its caller must commit the exact independent verification delivery attempt before this RPC.
// A lost response requires receipt observation, never automatic redelivery.
func (h *Handler) PublishNetbirdRemovalAbsence(parent context.Context, c netbirdcommand.Command) (*netbirdcommand.Receipt, error) {
	if parent == nil || c.Version != netbirdcommand.RemovalAbsenceVersion || !c.Executable(c.Identity, time.Now()) {
