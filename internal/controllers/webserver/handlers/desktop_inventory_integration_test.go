...
func exerciseDesktopInventoryPermissions(t *testing.T, h *Handler, ctx context.Context, tenant, site, sibling, otherTenant, otherSite int, request func(string, string, string, url.Values) *httptest.ResponseRecorder) {
	...
	for _, user := range []string{"scoped-viewer", "scoped-operator", "organization-admin"} {
		for _, prefix := range []string{"", fmt.Sprintf("/tenant/%d", tenant), base} {
			for _, suffix := range []string{"", "/overview", "/hardware", "/os"} {
				...
				for _, forbidden := range []string{"private-inventory-", "Private organization", "Private site", "endpoint-description", "endpoint-type", "Confirm deletion", "/computers/windows-fixture/power", "<dd>finance</dd>"} {
	...
	for _, user := range []string{"scoped-viewer", "scoped-operator", "organization-admin"} {
		for _, endpoint := range []struct{ method, suffix string }{{"POST", "/overview"}, {"DELETE", ""}, {"GET", "/remote-assistance"}, {"GET", "/power"}, {"POST", "/software"}, {"POST", "/network-adapters"}, {"POST", "/inventory/network"}, {"POST", "/physical-disks"}, {"POST", "/inventory/storage"}, {"POST", "/logical-disks"}, {"POST", "/logical-disks/file"}, {"PUT", "/logical-disks/file"}, {"DELETE", "/logical-disks/file"}, {"POST", "/logical-disks/downloadfile"}, {"POST", "/logical-disks/downloadfolder"}, {"POST", "/logical-disks/downloadmany"}, {"POST", "/logical-disks/folder"}, {"PUT", "/logical-disks/folder"}, {"DELETE", "/logical-disks/folder"}, {"DELETE", "/logical-disks/many"}, {"POST", "/hardware"}, {"POST", "/os"}, {"POST", "/printers"}, {"POST", "/monitors"}, {"POST", "/inventory/peripherals"}, {"POST", "/inventory/memory"}, {"POST", "/inventory/shares"}, {"POST", "/inventory/security"}, {"POST", "/shares"}} {
