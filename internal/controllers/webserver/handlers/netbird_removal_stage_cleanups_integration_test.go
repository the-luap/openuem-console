...
func exerciseNetbirdRemovalStageCleanupRoutes(t *testing.T, h *Handler, e *echo.Echo, ctx context.Context, tenant, site, foreign int, original netbirdcommand.Command, originalRelease string) {
	t.Helper()
	old := h.NetbirdRemovalStageCleanups
	defer func() { h.NetbirdRemovalStageCleanups = old }()
	originalHash, err := original.Digest()
	require.NoError(t, err)
	reference := netbirdcommand.RemovalStageCleanupReference{RequestID: original.RequestID, CommandHash: originalHash, Revision: original.Revision, ReleaseID: originalRelease}
	unavailable := false
	...
	control := func(_ context.Context, c netbirdcommand.ControlRequest) (*netbirdcommand.ControlResponse, error) {
		if c.Kind == "removal-stage-cleanup-state" {
			inspections++
			require.Equal(t, reference, c.RemovalStageCleanupOriginal)
			if unavailable {
			...
			p.RemovalStageCleanup = netbirdcommand.RemovalStageCleanup{Original: reference, Profile: netbirdcommand.RemovalStageCleanupProfile, JournalRevision: p.State.Revision, StateDigest: strings.Repeat("a", 64), DirectoryCount: 7, ManifestPresent: true, ManifestBytes: 128}
			return &p, err
		}
		if c.Kind == "state" {
			p, err := netbirdcommand.ControlResponseFor(c, "ok")
	...
	store, err := inventory.NewNetbirdRemovalStageCleanupDeliveryStore(h.Model.DB, h.Access, true, control, func(_ context.Context, c netbirdcommand.Command) (*netbirdcommand.Receipt, error) {
		native++
		require.Equal(t, netbirdcommand.RemovalStageCleanupVersion, c.Version)
		require.Equal(t, "cleanup-removal-stage", c.Operation)
		require.Equal(t, reference, c.RemovalStageCleanup.Original)
		require.Equal(t, 7, c.RemovalStageCleanup.DirectoryCount)
		require.True(t, c.RemovalStageCleanup.ManifestPresent)
		require.Equal(t, int64(128), c.RemovalStageCleanup.ManifestBytes)
		command = c
		if receiptState != "completed" {
			return nil, errors.New("owned lost cleanup reply")
		}
		out, err := netbirdcommand.ReceiptFor(c, "completed")
		return &out, err
	})
	require.NoError(t, err)
	h.NetbirdRemovalStageCleanups = store
	request := ownedTagHTTPRequest(t, h, e, ctx)
	path := fmt.Sprintf("/tenant/%d/site/%d/computers/%s/netbird/removal-stage-cleanups", tenant, site, original.DeviceID)
	...
	unavailable = true
	unavailableReview := request("scoped-operator", "GET", path+"/review?original_id="+original.RequestID, nil, "")
	require.NotEqual(t, 200, unavailableReview.Code)
	require.NotContains(t, unavailableReview.Body.String(), "Queue staging cleanup")
	unavailable = false
	...
	for _, change := range []string{"confirmation", "duplicate", "unknown", "revision", "query", "original"} {
		...
		case "query":
			target += "?stage_cleanup_digest=" + strings.Repeat("a", 64)
		}
	...
	require.Equal(t, 200, response.Code, response.Body.String())
	require.NotContains(t, response.Body.String(), "Cancel cleanup request")
	require.Contains(t, response.Body.String(), "Queued for staging cleanup")
	...
	for _, scenario := range []string{"completed", "observe", "withdraw", "release"} {
		...
		if scenario == "observe" {
			...
			resolution := request("scoped-operator", "GET", receiptPath+"/resolution", nil, "")
			require.Equal(t, 200, resolution.Code)
			resolved := fields(resolution.Body.String())
		...
		if scenario == "completed" || scenario == "observe" {
			require.Contains(t, response.Body.String(), "Reviewed staging cleanup completed")
		} else {
	...
	require.NotContains(t, request("scoped-viewer", "GET", originalPath, nil, "").Body.String(), "Review retained staging cleanup")
	var outcome string
