...
func (h *Handler) NetbirdResolutionRetry(c echo.Context) error {
	v, err := netbirdValues(c, "resolution_id", "retry_id", "revision")
	if err != nil {
		return netbirdFailure(err)
	}
	info, scope, err := h.netbirdInfo(c)
	...
	_, err = h.NetbirdResolutions.Retry(c.Request().Context(), info.Principal.UserID, scope, c.Param("uuid"), c.Param("request"), v.Get("resolution_id"), v.Get("retry_id"), v.Get("revision"))
	if err != nil {
		return netbirdFailure(err)
	}
	return netbirdRedirect(c, desktop_views.NetbirdOperationPath(info, c.Param("uuid"))+"/"+c.Param("request")+"/resolution")
