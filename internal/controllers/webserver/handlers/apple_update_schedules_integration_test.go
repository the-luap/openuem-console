...
func exerciseAppleUpdateSchedules(t *testing.T, h *Handler, ctx context.Context, tenant, site int, request func(string, string, string, url.Values) *httptest.ResponseRecorder) {
	t.Helper()
	scope := apple.Scope{TenantID: tenant, SiteID: site}
	sources := inventory.DeviceSources{Apple: true, Windows: h.Windows != nil}
	invite, err := h.Apple.Invite(ctx, scope, "Owned schedule update target", "apple-console-admin")
	...
	require.NoError(t, err)
	plan, err := h.Apple.SaveUpdatePlan(ctx, "scoped-operator", h.Access, scope, "", 0, apple.UpdatePlanDefinition{Name: "Owned <scheduled update>", Platform: "ios", TargetVersion: "18.7.1", TargetBuild: "22H100", Deadline: "2026-10-01T18:00:00"})
	require.NoError(t, err)
	group, err := inventory.SaveDeviceGroup(ctx, h.Model.DB, h.Access, "scoped-operator", access.Scope{TenantID: tenant, SiteID: site}, "", 0, inventory.DeviceGroupDefinition{Name: "Owned <scheduled group>", Rule: inventory.DeviceGroupRule{Search: "Owned schedule update target"}})
	require.NoError(t, err)
	...
	form := url.Values{"expected_revision": {"1"}, "group_id": {group.ID}, "group_revision": {"1"}, "request_key": {uuid.NewString()}, "devices": {invite.DeviceID + ":" + preview.Targets[0].Selection.PolicyToken}, "not_before": {time.Now().UTC().Add(time.Hour).Format("2006-01-02T15:04:05Z")}, "activation_window_minutes": {"60"}}
	path := base + "/schedules"
	...
	saved := request("scoped-operator", "POST", path, form)
	...
	require.Contains(t, detail.Body.String(), "Owned &lt;scheduled update&gt;")
	require.Contains(t, detail.Body.String(), "data-update-schedule-cancel")
	...
	require.Equal(t, 403, request("scoped-viewer", "POST", location+"/cancel", cancel).Code)
	...
	require.Contains(t, request("scoped-operator", "GET", location, nil).Body.String(), "Canceled")
	form.Set("request_key", uuid.NewString())
	form.Set("not_before", time.Now().UTC().Format("2006-01-02T15:04:05Z"))
	due := request("scoped-operator", "POST", path, form)
	require.Equal(t, 303, due.Code)
	progress, err := h.Apple.ProcessDueUpdateSchedules(ctx, h.Access, sources, 25)
	require.NoError(t, err)
	require.Equal(t, 1, progress.Activated)
	detail = request("scoped-operator", "GET", due.Header().Get("Location"), nil)
	require.Equal(t, 200, detail.Code)
	require.Contains(t, detail.Body.String(), "Open activation receipt")
	require.NotContains(t, detail.Body.String(), "data-update-schedule-cancel")
	require.Equal(t, 200, request("scoped-operator", "GET", path, nil).Code)
	form.Add("activation_window_minutes", "60")
	require.Equal(t, 400, request("scoped-operator", "POST", path, form).Code)
}
