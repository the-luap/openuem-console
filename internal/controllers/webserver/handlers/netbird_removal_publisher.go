...
// PublishNetbirdRemoval accepts only a fresh native removal envelope.
// Its native-state inventory caller must have committed the exact delivery
// attempt. Lost responses require receipt observation, never another delivery.
func (h *Handler) PublishNetbirdRemoval(parent context.Context, c netbirdcommand.Command) (*netbirdcommand.Receipt, error) {
	if parent == nil || c.Version != netbirdcommand.RemovalVersion || !c.Executable(c.Identity, time.Now()) {
