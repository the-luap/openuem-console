    ...
    for (const width of [390, 768, 1440]) {
      ...
      const result = await browser.evaluate(`(() => {
        ...
        return {rows:root.querySelectorAll('tbody tr').length,post:root.querySelectorAll('form[method="post"]').length,
          links:devicePageLinks.map(a=>({text:a.textContent,path:new URL(a.href).pathname,search:new URL(a.href).searchParams.get('q'),platform:new URL(a.href).searchParams.get('platform'),after:new URL(a.href).searchParams.get('after')})),
          search:form.elements.q.value,platform:form.elements.platform.value,afterField:!!form.elements.after,
      ...
      browser.check(result.search === "Owned" && result.platform === "ipados" && !result.afterField, "Filtering failed to retain values or reset the page cursor");
      browser.check(result.links.every(a => a.path === "/tenant/1/site/1/devices" && a.search === "Owned" && a.platform === "ipados"), "Page links lost scope or filters");
