export default async function run(browser, record) {
  ...
  for (const state of ["choose", "empty", "form", "preview", "history", "long", "schedule-choose", "schedule-form", "schedule-preview", "schedule-history", "schedule-group-changed", "schedule-sources-changed"]) {
    const scheduled=state.startsWith("schedule-");
    const baseState=state.replace(/^schedule-/,"");
    const action=scheduled?"schedule":"assign";
    for (const width of [390, 768, 1440]) {
      ...
      const view = await browser.evaluate(`(() => {
        ...
        return {width:document.documentElement.scrollWidth,viewport:innerWidth,scripts:root.querySelectorAll('script').length,
          text:root.textContent,source:root.querySelector('[data-group-source]')?.textContent,
      ...
      const chooser = baseState === "choose" || baseState === "empty";
      if (chooser) {
        browser.check(view.choices.length === (baseState === "empty" ? 0 : 1), "Archived groups became selectable or the empty page has targets");
        browser.check(view.choices.every(a => a.id === groupID && a.revision === "3" && a.path.endsWith("/"+action)), "Group choice lost its exact source revision");
        ...
        if (["form","preview","long"].includes(baseState)) {
          ...
          browser.check(view.method === "post" && view.path.endsWith("/"+action+(baseState === "form" ? "/preview" : "/create")), "Group assignment uses the wrong action");
          if (baseState === "form") browser.check(view.readonly, "Group target IDs can be edited independently of their source");
          ...
          if (baseState !== "form") {
            ...
            await browser.evaluate(`groupActionForm.elements.${scheduled?'confirm_schedule':'confirm_assignment'}.focus()`);
            await browser.space();
          }
          ...
          if(scheduled)browser.check(fields.not_before === "2026-09-12T13:00" && fields.activation_minutes === "60", "Group schedule lost its reviewed UTC activation window");
      ...
      if(baseState.endsWith("changed"))browser.check(view.text.includes("before activation") && view.text.includes("create a new plan"), "Blocked group schedule lost its cause and recovery guidance");
