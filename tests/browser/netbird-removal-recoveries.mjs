...
const base='/tenant/1/site/2/computers/90000000-0000-4000-8000-000000000009/netbird/removal-recoveries';
...
export default async function netbirdRemovalRecoveries(browser,record){
 for(const width of [390,768,1440]){
  for(const kind of kinds){
   await browser.visit('netbird-removal-recoveries-'+kind,width);
   ...
   if(kind==='released')browser.check(await browser.evaluate(`document.body.textContent.includes('unconfirmed')&&document.body.textContent.includes('does not confirm that this continuation completed')&&!document.body.textContent.includes('Package removal verified')`),'Reviewed release rewrote original removal evidence');
   ...
   if(confirmationKinds.includes(kind)){
    ...
    if(kind.startsWith('review'))browser.check(result.path===base&&f.request_id===id&&f.recovery_digest==='b'.repeat(64)&&f.original_id==='60000000-0000-4000-8000-000000000006'&&result.fields.length===6,'Continuation request lost exact installed state/review/UUID');
   ...
   if(width===390&&['review-long','released','resolution-long'].includes(kind))await browser.capture('netbird-removal-recoveries-'+kind+'-390');
   record({name:'NetBird removal continuations '+kind,width,passed:true});
  }
  for(const kind of ['review','resolution-withdraw','resolution-release']){
   ...
   record({name:'NetBird removal continuations '+kind+' pending',width,passed:true});
  }
 }
}
