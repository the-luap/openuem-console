...
export default async function netbirdRemovalAbsences(browser,record){
 for(const width of [390,768,1440]){
  for(const kind of kinds){
   await browser.visit('netbird-removal-absences-'+kind,width);
   browser.check(await browser.evaluate('document.documentElement.scrollWidth<=innerWidth+1'),'Absence check layout overflows at '+kind);
   browser.check(await browser.evaluate(`document.querySelectorAll('#netbird-operation-heading').length===1&&!document.querySelector('native')&&!document.querySelector('operator')`),'Absence check metadata became markup');
   ...
   if(kind==='completed')browser.check(await browser.evaluate(`document.body.textContent.includes('Current package absence verified')&&document.body.textContent.includes('The original uninstall retains its own outcome')`),'Absence verification changed original outcome');
   if(kind.startsWith('review'))browser.check(await browser.evaluate(`document.body.textContent.includes('This check reads the current state. It cannot remove files or stop processes.')`),'Review loses read-only scope');
   if(kind==='released')browser.check(await browser.evaluate(`document.body.textContent.includes('unconfirmed')&&document.body.textContent.includes('does not confirm that this verification completed')&&!document.body.textContent.includes('Current package absence verified')`),'Reviewed release rewrote original removal evidence');
   ...
   if(confirmationKinds.includes(kind)){
    ...
    if(kind.startsWith('review'))browser.check(result.path===base&&f.request_id===id&&f.absence_digest==='b'.repeat(64)&&f.original_id==='60000000-0000-4000-8000-000000000006'&&result.fields.length===6,'Continuation request lost exact absence state/review/UUID');
   ...
   if(width===390&&['review-long','released','resolution-long'].includes(kind))await browser.capture('netbird-removal-absences-'+kind+'-390');
   record({name:'NetBird removal verifications '+kind,width,passed:true});
  }
  for(const kind of ['review','resolution-withdraw','resolution-release']){
   ...
   record({name:'NetBird removal verifications '+kind+' pending',width,passed:true});
  }
 }
}
