...
export default async function run(browser,record) {
 for(const state of ['reported','required','unverified','different','removed','unavailable','attention','mixed','long','deadline-elapsed','deadline-fold','deadline-stale','exception-active'])for(const width of [390,768,1440]){
  ...
  const view=await browser.evaluate(`(()=>{
   ...
   return {width:document.documentElement.scrollWidth,viewport:innerWidth,text:main.textContent,forms:main.querySelectorAll('form').length,devices:[...main.querySelectorAll('[data-update-progress-device]')].map(e=>({deadline:e.querySelector('[data-update-deadline]')?{state:e.querySelector('[data-update-deadline]').dataset.updateDeadline,text:e.querySelector('[data-update-deadline]').textContent}:null,id:e.dataset.updateProgressDevice,text:e.textContent,links:[...e.querySelectorAll('a')].map(a=>a.getAttribute('href'))})),deadlines:main.querySelector('[data-update-deadline-counts]').textContent,os:main.querySelector('[data-update-progress-counts]').textContent,policy:main.querySelector('[data-update-policy-counts]').textContent,links:[...main.querySelectorAll('a')].map(a=>({text:a.textContent,href:a.getAttribute('href')}))};
  })()`);
  ...
  if(['required','deadline-elapsed'].includes(state))browser.check(view.os.includes('Update still required: 1')&&first.text.includes('18.6.2'),'Required update hidden');
  ...
  checkDeadline(browser,state,first.deadline,true);
