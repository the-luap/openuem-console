const kinds=['review','review-long','review-no-manifest','review-max','queued','delivery-pending','unconfirmed','viewer','stopped','cancelled','completed','released','history','history-empty','history-full','resolution-withdraw','resolution-release','resolution-retry','resolution-waiting','resolution-conflict','resolution-confirm','resolution-completed','resolution-released','resolution-long'];
const base='/tenant/1/site/2/computers/90000000-0000-4000-8000-000000000009/netbird/removal-stage-cleanups';
const id='10000000-0000-4000-8000-000000000001';
const confirmationKinds=['review','review-long','review-no-manifest','review-max','resolution-withdraw','resolution-release','resolution-retry','resolution-long'];
export default async function netbirdRemovalStageCleanups(browser,record){
 for(const width of [390,768,1440]){
  for(const kind of kinds){
   await browser.visit('netbird-removal-stage-cleanups-'+kind,width);
   browser.check(await browser.evaluate('document.documentElement.scrollWidth<=innerWidth+1'),'Staging cleanup layout overflows at '+kind);
   browser.check(await browser.evaluate(`document.querySelectorAll('#netbird-operation-heading').length===1&&!document.querySelector('native')&&!document.querySelector('operator')`),'Staging cleanup metadata became markup');
   ...
   if(kind==='completed')browser.check(await browser.evaluate(`document.body.textContent.includes('Reviewed staging cleanup completed')&&document.body.textContent.includes('The original uninstall retains its own outcome')`),'Staging cleanup changed original outcome');
   if(kind.startsWith('review'))browser.check(await browser.evaluate(`document.body.textContent.includes('This action removes the reviewed empty directories and any listed incomplete manifest metadata.')&&document.body.textContent.includes('Unknown files, package payloads and running processes prevent cleanup.')`),'Review loses exact destructive scope');
   if(kind==='review-no-manifest')browser.check(await browser.evaluate(`document.body.textContent.includes('None')&&!document.body.textContent.includes('128 bytes')`),'Absent metadata is displayed as present');
   if(kind==='review-max')browser.check(await browser.evaluate(`document.body.textContent.includes('2097152 bytes')`),'Maximum reviewed metadata size was lost');
   if(kind==='released')browser.check(await browser.evaluate(`document.body.textContent.includes('unconfirmed')&&document.body.textContent.includes('does not confirm that this cleanup completed')&&!document.body.textContent.includes('Reviewed staging cleanup completed')`),'Reviewed release rewrote original removal evidence');
   ...
   if(confirmationKinds.includes(kind)){
    ...
    if(kind.startsWith('review'))browser.check(result.path===base&&f.request_id===id&&f.stage_cleanup_digest==='b'.repeat(64)&&f.original_id==='60000000-0000-4000-8000-000000000006'&&result.fields.length===6,'Continuation request lost exact cleanup scope/review/UUID');
   ...
   if(width===390&&['review-long','released','resolution-long','review-no-manifest','review-max'].includes(kind))await browser.capture('netbird-removal-stage-cleanups-'+kind+'-390');
   record({name:'NetBird staging cleanup '+kind,width,passed:true});
  }
  for(const kind of ['review','resolution-withdraw','resolution-release']){
   ...
   record({name:'NetBird staging cleanup '+kind+' pending',width,passed:true});
  }
 }
}
