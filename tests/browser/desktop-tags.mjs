export default async function run(browser, record) {
  for (const width of [390, 768, 1440]) {
    ...
    const paths=['/tenant/1/site/2/agents','/tenant/1/site/2/computers','/tenant/1/admin/update-agents','/profiles/17/tags','/tenant/1/profiles/17/tags','/tenant/1/site/2/profiles/17/tags'];
    for (let i=0;i<paths.length;i++) {
      for (const method of i===2?['delete']:['post','delete']) {
        ...
        browser.check(r.fields.agentId===(i>=3?'17':'owned-agent') && r.fields.tagId==='42', 'Tag action lost its target identity');
      ...
      record({name:'Tag membership HTMX '+paths[i],width,passed:true});
    }
  }
}
