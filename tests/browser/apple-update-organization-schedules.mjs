export default async function run(browser,record) {
 for(const state of ['scheduled','waiting','activated','blocked','expired','canceled','history','empty-history','long','site-operator','reader'])for(const width of [390,768,1440]){
  await browser.visit('apple-update-schedule-organization-'+state,width);
  ...
  if(['scheduled','waiting','site-operator'].includes(state)){
  ...
  if(!['history','empty-history'].includes(state)) {
   browser.check(view.text.includes("Only the selected target site's members are included"),'Schedule lost original organization source');
   const sourceLinks=view.links.filter(a=>a.text==='Open the current organization group and its history');
   browser.check(sourceLinks.length===(['site-operator','reader'].includes(state)?0:1)&&sourceLinks.every(a=>a.href==='/tenant/1/device-groups/30000000-0000-0000-0000-000000000001'),'Historical schedule source link bypasses current organization rights or uses wrong site');
   browser.check(!view.links.some(a=>a.href==='/tenant/1/site/1/device-groups/30000000-0000-0000-0000-000000000001'),'Organization schedule group resolved under target site');
  }
  if(state==='history')browser.check(view.text.includes('Source: organization group · Target site: 1'),'History loses schedule source kind');
  if(width===390)await browser.capture('apple-update-schedule-organization-'+state+'-390');record({name:'Apple organization update schedule '+state,width,passed:true});
 }
}
