export default async function run(browser, record) {
  ...
  for (const state of ["empty", "pending", "other-owner", "reader", "site", "expired", "withdrawn", "approved", "focused", "paged", "review", "incompatible", "review-expired", "review-withdrawn", "review-approved", "derived"]) {
    for (const width of [390, 768, 1440]) {
      ...
      check(await evaluate("!!document.querySelector('main input[name=approval_id]')") === (state === "review"), "Source approval escaped its live compatible review");
      ...
      if (captureForm || state === "review") {
        ...
        if (captureForm) {
          ...
          check(await evaluate("document.querySelector('main').textContent.includes('separate MSI revision') && document.querySelector('main').textContent.includes('later confirmation')"), "Approval omitted its effect");
        }
      } else {
      ...
      if (width === 390 && ["pending", "review", "derived"].includes(state)) {
