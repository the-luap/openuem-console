export default async function run(browser, record) {
  ...
  for(const role of ['viewer','operator','organization_admin','administrator'])
    for(const width of [390,768,1440]){
      ...
      if(role==='administrator'||role==='organization_admin'){expected['Organization tags']='/admin/tags';expected['Custom fields']='/admin/metadata';}
      check(opened.links.length===Object.keys(expected).length && opened.links.every(a=>a.visible && a.path===((a.text==='Organization tags'||a.text==='Custom fields')?'/tenant/1':'/tenant/1/site/1')+expected[a.text]),'Management links changed permission, visibility or scope: '+JSON.stringify(opened.links));
      await tab();
