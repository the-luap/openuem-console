export default async function run(browser,record) {
  for(const width of [390,768,1440]) {
    for(const [scope,prefix] of Object.entries({global:'',organization:'/tenant/1',site:'/tenant/1/site/2'})) {
      ...
      let q=await browser.evaluate('taskCreationRequest');browser.check(q&&q.method==='get'&&q.path===prefix+'/tasks/17/new/types'&&q.fields.length===1&&Object.fromEntries(q.fields)['task-agent-type']==='windows','Task type lookup included unrelated form values');
      ...
      q=await browser.evaluate('taskCreationRequest');browser.check(q&&q.method==='get'&&q.path===prefix+'/tasks/17/new/subtypes'&&q.fields.length===1&&Object.fromEntries(q.fields)['task-type']==='powershell_type','Task subtype lookup included unrelated form values');
      ...
      if(scope!=='global') {
        ...
        q=await browser.evaluate('taskCreationRequest');browser.check(q&&q.path===prefix+'/tasks/17/new/definition'&&q.fields.length===1&&Object.fromEntries(q.fields)['task-subtype']==='netbird_register','NetBird definition lookup lost its profile scope or included unrelated values');
        await browser.evaluate(`htmx.swap('#task-definition',document.querySelector('#owned-netbird-definition').innerHTML,{swapStyle:'innerHTML'});`);
