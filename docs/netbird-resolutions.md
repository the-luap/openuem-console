...
| Missing receipt with explicit version-two support and ready journal capacity | Explicitly request permanent withdrawal of the original command. |
| Unsupported recovery, unavailable/full journal, active/unjoined execution or another resolution identity | Keep further commands blocked. |
...
The full browser matrix now passes 2,343 cases. Fixtures use owned callbacks and
...
audit suite passes in 8.774 seconds and the affected view race test in 1.924
seconds. Final registered-route fixtures and the full Linux console build pass.

[Permanent connection withdrawal](netbird-connection-withdrawals.md) now handles
missing receipts when the current agent proves support and has journal capacity.
Missing or unavailable journal evidence remains blocked. If the release request itself was
