...
| APP-01 | Instance-generated public CSR and encrypted per-request key; authorized request history/download/revocation; certificate-only local import; atomic renewal revision/topic checks, account metadata and setup-page expiry warning; PostgreSQL/console route tests | Authorized vendor integration and signed portal request verification/download; Apple issuer-chain validation and APNs connectivity before activation; complete renewal reminders and actual Apple issuance/device continuity |
...
- [Apple push requests](apple-push-requests.md) add a local external-vendor CSR
  ...
  a completed vendor signer or verified APNs setup wizard; APP-01 remains open.
