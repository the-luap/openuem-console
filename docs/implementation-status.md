...
| IOS-01 | Native iPhone/iPad protocol/profile/DDM foundation; reviewed site/organization-group profile assignments within explicit sites; versioned update plans, exact site/organization-source update admission and scheduled activation; current OS/deadline assessments, temporary exceptions and reviewed group removal; opt-in durable console incidents and acknowledgments; explicit all-original pilot readiness and atomic reviewed promotion with immutable encrypted evidence and PostgreSQL/router/browser coverage | iPad filters/templates and separate hardware evidence; full template targeting/conflicts/rollback; continuous group reconciliation, automatic promotion and external escalation channels |
...
- [Desktop device details](desktop-device-details.md) now combine display name,
  ...
  preserve every submitted field beside current saved values. Existing direct
  overview/nickname forms cannot write without the editor's revision. Reads and
  writes commit with current grants, complete membership locks and inventory
  audit events; the audit excludes field contents. Assignment reviews bind the
  new details revision as well. Cleared display names retain hostname/ID fallback
  in the computer list and headers, and unbroken names wrap in the shared
  breadcrumb header. All 453 inventory test/fuzz families pass across eight
  race-test partitions; the complete registered console route fixture passes in
...
- [Protected Linux bootstrap staging](https://github.com/the-luap/openuem-agent/blob/c76ca95fc22d263fd224ac550624c9383102eabc/docs/linux-package-staging.md)
  ...
  Unsafe ancestors fail before downloads or directory creation; rechecks bind
  the original private file to the current checkpoint, lifetime and signed bytes.
  Descriptor-relative cleanup preserves replaced namespaces and unknown files.
...
- Console CI `34879348764` passes all four Apple protocol partitions and both
  ...
  eight separate PostgreSQL/race jobs, retaining that per-run timeout. The shared
  ...
  contain 55/55/55/55/55/54/54/54 entries; no test family is removed from coverage.
  All 437 tests pass locally in four complete isolated partitions in
  437.126/490.657/423.886/382.336 seconds. Hosted runners still exceed ten minutes
  on two of those larger partitions, so CI uses the eight smaller groups. Each
  job generates its required console templates and owns an independent PostgreSQL
...
- [Native NetBird removal protocol and journal admission](netbird-removal-commands.md)
  ...
  result loss, current-certificate observation and common exclusion. [Reviewed
  ...
  [Private retained-stage recovery components](https://github.com/the-luap/openuem-agent/blob/bf9f98969a5ba134eab504da9fd30b2539bde39d/docs/netbird-removal-staging-recovery.md)
  ...
  distinct from native completion. The combined observer now adds repeated typed
  service state, original/relocated audit-token code proofs and native receipt
  ...
  A private single-use owner now joins remaining-source moves, exact purge,
...
- [Scheduled Apple updates from organization groups](apple-update-organization-schedules.md)
  ...
  source read and target update rights before activation; source revisions,
  membership, policies and current grant revision must still match. Child source
  scope must match its parent. Site operators retain historical read/cancellation
  rights, while live organization links require current organization read rights.
  The complete schedule PostgreSQL/race regression passes in 41.795 seconds,
  including concurrent activation, changed inputs, final-audit rollback and locks,
  old/new source formats, state binding and opposite-source child substitution.
  Registered Linux routes and 183 relevant Chrome cases pass, including 33 new
  organization schedule states and both immediate/scheduled keyboard confirmation.
  Actual legacy version-1 activation passes separately in 2.958 seconds.
  The full 1,239-case Chrome matrix passes; mobile pending/activated schedules
  were inspected. Linux/macOS package race checks and the full Linux build pass.
...
- [Apple profiles from organization groups](apple-profile-organization-groups.md)
  ...
  target site. Source-wide read authority and target action authority remain
  distinct; SQL evaluates only the site's members before the 100-member bound.
  Version-2 encrypted receipts retain source scope while reading original
  version-1 site receipts unchanged. Chooser, preview, apply/removal, replay and
  historical source links preserve the exact source kind and selected targets.
  Organization-only routes reject fallback to a default site. Source/group,
  target site/device and current permission locks remain held through admission
  and final audit. Inventory PostgreSQL 17/race tests pass in 5.390 seconds;
  final profile-group tests pass in 16.664 seconds, including late-audit rollback
  and lock ordering. The profile-group/pilot-promotion regression passes in
  53.801 seconds. Registered Linux routes and 156 relevant Chrome cases pass,
  including 30 new organization-source cases; mobile preview and receipt were
  inspected. The full browser runner defines 1,170 cases.
  macOS/Linux package race checks and the full Linux build pass.
  The full 1,170-case Chrome matrix and four startup lifecycle checks also pass.
  A previous CI run exhausted the old three-minute global browser deadline.
...
- [Reviewed Apple pilot promotion](apple-update-promotions.md) now verifies
  ...
  destination group with the same platform/version/build. Fresh post-admission
  OS evidence, matching healthy policy values, valid identities and ended
  exceptions are checked under current authority and canonical locks covering
  both cohorts. Overlapping pilot devices remain explicit destination targets;
  at least one eligible new enrollment is required. Pilot evidence, destination
  policies/notifications, both immutable receipts and audits commit together.
  Final identity expiry or stale proof rolls everything back. Exact retries
  return the original receipt without restoring later policy state. Encrypted
  source-bound history retains original proof through plan archival and device
  movement, with 25-entry authenticated keyset pages.
  The combined pilot/promotion PostgreSQL 17/race tests pass in 54.554 seconds.
  All 33 strict form cases pass, including a complete 100-device selection.
  Actual registered Linux routes, macOS/Linux package race tests and the full
  Linux build pass. All 378 relevant Chrome cases pass, including 60 new pilot
  cases; those 60 also pass after the final fixture correction. Mobile overlap,
  confirmation and saved-evidence views were inspected. The complete browser
  runner now defines 1,140 cases; this change ran the relevant 378.
  The complete Apple PostgreSQL/race suite passes in 713.737 seconds.
  Automatic promotion, continuous group
  reconciliation, external escalation channels and physical-device acceptance
  remain separate work. Existing direct assignments remain independent actions.

- [Durable Apple console update monitoring](apple-update-escalations.md) now
  retains the immutable original cohort, conservative deadline/OS/exception
  evidence, stable incidents and explicit operator acknowledgments. Opt-in
  enable/pause/rearm actions and immutable encrypted history preserve exact
  retries and later state. Bounded background checks retain current authority
  locks, stop on owner permission revision changes, and commit state, history and
  audit together. Unknown evidence retains an existing incident; policy changes
  and exceptions remain distinct from installation. Scoped review/list/event
  routes, strict forms, responsive keyboard-confirmed pages and joined runtime
  workers are implemented. The escalation/progress/exception/deadline PostgreSQL
  17/race regression passes in 47.616 seconds. A separate final-audit device and
  authority lock test passes in 3.227 seconds; PostgreSQL maintenance lifecycle
  checks pass in 2.327 seconds. Receipt context-substitution checks pass in
  1.568 seconds. All 56 strict form cases pass.
  Registered Linux routes, macOS/Linux package race tests and the full Linux
  build pass. All 318 relevant Chrome cases pass, including 54 new monitoring
  cases; mobile attention and source-blocked views were inspected. The complete
  browser runner defines 1,080 cases; this change reran the relevant 318.
  The complete Apple PostgreSQL/race suite passes in 644.195 seconds; the
  additional lock and receipt-context tests above were run separately after
  that suite started. External notification channels, continuous group
  reconciliation, pilot promotion and
...
- [Apple update assignments from dynamic groups](apple-update-groups.md) now
  ...
  values before atomically replacing policies, queuing declarations and retaining
  immutable encrypted original requests. Exact replay after archival, manual
  removal or later observations returns original evidence without rearming work.
...
- [Scheduled Windows group assignments](native-windows-update-groups.md) now
  ...
  three and blocks changed intent without creating a cohort. Original group
  metadata remains bound to the activated plan during device delivery. Exact
  request replay preserves terminal history after group or configuration changes.
  The console includes group selection, timing review, explicit confirmation,
  original-source history and localized blocked reasons. The complete actual
  Linux PostgreSQL 17/race suite passes in 190.222 seconds; registered console
...
- [Immediate Windows ring assignments from dynamic groups](native-windows-update-groups.md)
  ...
  group provenance cannot reuse its request as another operation. This initial
  change covered immediate assignments; the scheduled extension is recorded above.
...
- [Dynamic device groups](dynamic-device-groups.md) now persist exact-scope
  ...
  exceptions, conflict checks, automatic promotion
...
- [Scoped security inventory](desktop-security-inventory.md) now exposes
  ...
  custom status strings and exact timestamps retain separate meaning; collector
  zero dates display as unreported. Support URLs remain text. PostgreSQL
  inventory/audit race suites pass in 20.982/8.243 seconds, including eight-action
  export/retention. The additional 50,000-entry foreign-history/status regression
  passes in 2.529 seconds. Actual Linux ARM64 routes, all view race suites and
  the full console build pass. Thirty-six new browser cases cover six report
  states for viewers/operators at 390/768/1440 pixels; all 660 cases pass.
  Physical collection and current protection/patch acceptance remain open.

- [Scoped share inventory](desktop-shares-inventory.md) now exposes reported
  names, descriptions and paths, including delegated legacy GET aliases for
  scoped readers. Literal search and 25-entry continuation retain exact current
  membership, transaction-bound authorization and committed read audits. Paths
  remain escaped text and grant no file access. PostgreSQL inventory/audit race
  suites pass in 17.280/7.708 seconds, including seven-action export/retention
  and 50,000 foreign entries. Actual Linux ARM64 routes, all view race suites
  and the full console build pass. Twenty-four new browser cases cover report
  states, roles, literal UNC/script-like paths and keyboard search; all 624
  cases pass. Physical collection and remaining legacy actions stay open.
...
- [Scoped memory-module inventory](desktop-memory-inventory.md) now exposes
  ...
  Exact current device membership and transaction authorization reject foreign,
  ambiguous and revoked access. PostgreSQL inventory and audit race suites pass
  in 16.589/7.786 seconds, and six-action audit export/retention passes in 1.865
  seconds. Real Linux ARM64 routes, desktop view race tests and full console
  compilation pass. Twenty-four new viewer/operator browser cases preserve
  missing and literal values at 390/768/1440 pixels; all 588 cases pass. Physical
  collection and the remaining legacy detail/action permissions stay open.

- [Scoped peripheral inventory](desktop-peripherals-inventory.md) now provides
  monitor and printer reports to scoped readers, including delegated legacy GET
  aliases. Literal search, 25-entry continuation, exact current membership and
  a committed read audit protect both kinds. Printer flags retain absent/false/
  true evidence and reported ports remain escaped text. PostgreSQL inventory
  and audit race suites pass in 14.647/7.902 seconds; the five-action audit export
  and retention regression passes in 1.820 seconds. Actual Linux ARM64 routes,
  filter race checks, desktop view race tests and full console compilation pass.
  Forty-eight new viewer/operator browser cases pass at 390/768/1440 pixels;
  all 564 cases are green. This extends scoped reads; remaining legacy actions
  and physical peripheral collection remain separate acceptance work.
...
- [Scoped storage inventory](desktop-storage-inventory.md) now provides physical
  ...
  device membership, current transaction authorization and committed read audit
  protect both report kinds. Missing strings, stored zero usage and invalid
  percentages preserve their meaning; BitLocker text is reported evidence.
  PostgreSQL inventory/audit race suites pass in 14.241/8.702 seconds, and the
  extended four-action audit export/retention regression passes in 1.975 seconds.
  Real Linux ARM64 routes cover all three prefixes, continuation, foreign
  objects, invalid queries and denied file actions. All view race tests, filter
  race checks and the full console build pass. The 48 new browser cases pass at
  390/768/1440 pixels, with all 510 cases green. Other legacy detail/action
  permissions and physical endpoint acceptance remain open.
...
- [Localized report timestamps](localized-report-times.md) now use the selected
  ...
  `datetime`, audit seconds are preserved, missing evidence retains its label,
  and the server text remains usable without browser formatting. Real DOM
  insertion, seven locales, fixed offsets, daylight-saving boundaries, invalid
  values and unavailable Intl formatting pass 21 dedicated browser cases. The
  complete 408-case suite passes after checking label/time spacing. All view
  race suites, the real Linux ARM64 router and full console compilation pass.
  Device-local deadlines, request signatures, filters and exports retain their
  original semantics; remaining legacy date helpers and number/state formatting
  stay open under UX-01.

- [Account display language](account-language.md) now persists an optional
  selection per account and applies it after session loading on each request.
  Browser defaults, existing sessions, new logins, account isolation, production
  CSRF/Origin checks and unavailable-storage fallback have PostgreSQL and real
  console-router evidence. Shared documents identify their active language, and
  the profile forms wrap on narrow screens. All view and router middleware race
  suites pass. The new 30-case browser matrix covers the seven bundled catalogs,
  browser default, unavailable storage and long profile values; the complete
  browser suite passes 387 cases. Full Linux ARM64 console compilation passes.
  Full translation, dates/numbers and broader UX-01 acceptance remain open.

- [Scoped network inventory](desktop-network-inventory.md) now supports viewers,
  operators and administrators, including the delegated legacy GET alias. The
  single-statement report projection, current transaction authorization and
  committed read audit reject hidden devices, stale scope and audit failures.
  Literal name/MAC/IP search and 25-entry cursor pages preserve nullable stored
  flags and render all reported values as text. PostgreSQL inventory and shared
  audit race suites pass in 11.428/7.590 seconds, the real Linux ARM64 console
  router and full console build pass, and all view race tests pass. The new 24-case viewer/operator
  browser matrix passes at 390/768/1440 pixels; the complete suite now passes 357
  cases. This extends scoped inventory reads; other legacy detail/action
  permissions remain open under SEC-01.
...
- The legacy network-adapter page now renders reported DNS servers/domain as
  ...
  network browser cases pass; that revision's complete browser matrix passed 333 cases. The legacy
  server-administrator rendering remains available. See
...
- Native Windows package replacement and staging cleanup now pass on both
  ...
  no-op, harmless removal of the old revision and final successor removal in
  2.01 seconds on AMD64 and 2.19 seconds on ARM64. Repeated Burn cancellation
  exposed transient Windows deletion failures. Joined cleanup now rechecks the
  original file identity on each attempt under a shared two-second bound,
  preserves replacements and unexpected children, and retains sanitized failure
  across repeated Close calls. Native tests prove actual file/directory sharing
  conflicts and bounded permanent failure. All seven
  [agent CI jobs pass](https://github.com/the-luap/openuem-agent/actions/runs/34633746056),
  including three repetitions of each Burn install/remove and child-lifetime
  scenario. These disposable client/Server runners do not establish physical
  offline/restart/hibernate or every-edition acceptance.
  The complete console browser suite at `78fced1` passes 321 cases, including
  separate MSI and Burn source approval at 390/768/1440 pixels with native keyboard
  confirmation, exact identity/form fields and no overflow. Its shared browser
  assertions now retain the generic installer wording used by the MSI/Burn UI.
...
- [WinGet source review and MSI approval](windows-winget-resolution.md) now connect
  ...
  fixed Microsoft community commit, then approve a compatible machine MSI before
  the database-issued 15-minute deadline. The immutable link preserves its
  original coordinate and exact architecture/detection, and adds scoped source
  evidence to the derived MSI detail page. Permissions are rechecked after the
  network fetch; review, capture, publication and history require atomic audits.
  Concurrent retries, changed ownership, expired confirmation, generic approval
  ID reuse and inconsistent retained provenance are covered by PostgreSQL/race
  tests. The migration preserves existing approvals and inert preparations.
  Actual encrypted install/remove task tests preserve the source-derived plan
  digest. The complete Apple model race suite passes in 435.349 seconds; final
  capture-ID conflict checks pass with the targeted catalog/source tests in
  18.432 seconds. Source/MSI race
  tests in 1.846 seconds, full handler and rendered-view tests pass, and all 318
  browser cases pass (48 new source cases at 390/768/1440 pixels). Full Linux ARM64
  and Windows AMD64 builds and focused model/view/handler/translator vet pass. Further WinGet
  formats and physical package/offline/restart/hibernate acceptance remain open.
...
- [WinGet source snapshots](windows-winget-resolution.md) now bind the exact
  ...
  focused vet and Windows compilation also pass. Encrypted immutable approval
  linkage and scoped console review are now integrated as recorded above. Other
  installer kinds remain open; the standalone reader/translator does not itself
  authorize or dispatch software.
