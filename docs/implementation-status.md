...
| WIN-02 | Separate native discovery XML/SOAP codec and read-only TLS handler, OnPremise XCEP request decoding, scoped PostgreSQL enrollment credentials with permission revisions, revocation and atomic one-use consumption; synthetic protocol/TLS/credential race tests; production route not registered | XCEP policy responses and service integration, WSTEP certificate issuance, durable device identities/provisioning, credential and gateway/UI integration, SyncML/CSP policies/results, renewal/unenrollment and physical Windows acceptance; separate Entra/Autopilot integration evidence |
...
- [Native Windows discovery](native-windows-mdm.md) adds a bounded UTF-8 SOAP 1.2
  ...
  cancellation rollback. Credential service/UI integration, policy responses,
  verified CSR issuance and real Windows enrollment remain open.
