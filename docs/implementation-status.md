...
| WIN-02 | Separate native discovery XML/SOAP codec and read-only TLS handler, strict endpoint/parser/HTTP boundaries and synthetic local race tests; production route not registered | Authenticated XCEP/WSTEP enrollment, durable scoped identities/provisioning, gateway/UI integration, SyncML/CSP policies/results, renewal/unenrollment and physical Windows acceptance; separate Entra/Autopilot integration evidence |
