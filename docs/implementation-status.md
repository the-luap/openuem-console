...
| --- | --- | --- |
| NET-01 | Provider-neutral DNS-01 issuer with pinned lego, protected account identity and atomic publication; native HTTPS gateway with atomic public TLS file renewal, retained live streams and per-handshake lifetime checks; exact public Apple route allowlist; source-network admin restriction; pinned mutual TLS on all backends; canonical login origin; optional exact agent WSS, upgrade limits and stream shutdown tested against real NATS; optional desktop metadata/configuration/key/claim/download routes with pinned private TLS and bounded streaming; optional exact Windows discovery/XCEP/WSTEP/SyncML routes with pinned gateway identity; synthetic TLS/PostgreSQL tests | Released-agent authorization/enrollment integration; full reference installation/firewall proof; live DNS/ACME provider operations, proxy/browser/timeouts/load acceptance |
