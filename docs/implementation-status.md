...
| WIN-02 | Separate native discovery XML/SOAP codec and read-only TLS handler, OnPremise XCEP request decoding, strict endpoint/parser/HTTP boundaries and synthetic local race tests; production route not registered | Authenticated XCEP/WSTEP enrollment, durable scoped credentials/identities/provisioning, gateway/UI integration, SyncML/CSP policies/results, renewal/unenrollment and physical Windows acceptance; separate Entra/Autopilot integration evidence |
...
- [Native Windows discovery](native-windows-mdm.md) adds a bounded UTF-8 SOAP 1.2
  ...
  race tests pass locally; Linux/native Windows CI steps and a bounded fuzz run
  are added. Discovery is not registered publicly and does not yet authenticate
  ...
  redacts default credential serialization. Its synthetic race tests pass;
  credential validation, policy responses and certificate issuance remain open.
