...
[Custom metadata](custom-metadata.md) now uses organization-scoped field names,
a dedicated administrator capability, paged catalogs and revision-checked device
values. Conflict responses preserve drafts. Clearing a value and deleting a field
have explicit forms; field deletion requires an expiring impact review and an
immutable completion receipt. Current ownership, audit and changes by older
writers remain inside the transaction boundary. Console, worker and certificate
manager initializers preserve scoped uniqueness on restart. PostgreSQL race,
registered route and 45 responsive Chromium cases cover the new workflows.
Compatible worker/certificate initializers must precede the console migration.
...
fixtures cover isolation, bounds, conflicts, CSRF and rollback. Remaining desktop
actions still need their individual authorization work.
...
components. Expiring live state/receipt/release controls and a joined agent
service adapter are now implemented. The console dispatcher and direct publisher
